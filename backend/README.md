...
- `ending_room_service` passes `scenario_question` into roundtable static anchors and phase insights. Long or repeated question prefixes are stripped before phase insight compaction, so the visible one-line insight keeps the actual hinge instead of repeating the question.
