...
async def score_prediction(prediction_id: str) -> dict | None:
...
async def score_all_for_scenario(scenario_id: str) -> list[dict]:
    ...
    async def _score_with_limit(pred_id: str) -> dict | None:
        async with sem:
            return await score_prediction(pred_id)
