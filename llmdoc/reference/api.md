...
- daily challenge catalog 当前 52 条，weekly track registry 当前 7 条，badge registry 当前 15 条。`badge-definitions` 是静态 registry，不带用户进度；`profile/{user_id}/badges` 与 `profile/{user_id}/unlocks` 返回当前 user scope 的已解锁徽章。
