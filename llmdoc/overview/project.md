...
| Automation QA Hooks | 浏览器自动化钩子 — `render_game_to_text()` 覆盖关键页面，Theater 暴露 `advanceTime(ms)`；Simulation 页面新增 `page.replay_state`、`page.warmup`、`page.controls.capture_mode/can_capture_modal/capture_result_kind`，并通过 `capture_game_screenshot(panel|canvas|modal)` 提供显式截图模式；Track D 的 Debate live/result 现额外输出 `selected_phase / is_phase_locked / unlocked_phases / active_modal / modal_state / bet_window_open` 等状态摘要；前端同时提供 `e2e:matrix / e2e:variants / e2e:corners / e2e:full` 与 `e2e:debate:*` 一键回归入口，`e2e-suite.mjs` / `e2e-debate-suite.mjs` 都会在输出目录落盘 `browser-launch.json`，记录本次实际采用的浏览器启动 profile；`capture-modes` 现在会额外落盘 `predictionModalBytes / gameplayModalBytes`，`replay` corner case 会等到 `theater_ready = true` 再判定恢复完成；当前固定样本为 15 条，变体样本为 3 条；历史 `scenario_id` 缺失或 `scene_theme` 与当前 `select_scene(question)` 漂移时，suite 会按 theme runtime fallback 重建样本。Track C 主基线工件位于 `frontend/output/e2e/20260317-track-c/{matrix,corners,mobile,variants}/`；当前 full 主链路工件位于 `frontend/output/e2e/20260318-post-db-path-fix-full/`，结果页 backend summary smoke 工件位于 `frontend/output/e2e/20260318-result-backend-summary-smoke-v2/result-final.json`，Track D Debate 最新成功工件位于 `frontend/output/e2e/20260318-post-b2-debate-full/` |
...
| Vitest + jsdom | 本轮已验证前端全量回归 **175 passed**，覆盖状态管理、回放、截图、玩法卡、场景推断和页面自动化摘要；最近已真实执行的定向回归为 `gameplayContract / gameplayCards / GameplayCardsModal / scenarioMeta` 共 **26 passed**，以及这次文档同步重新执行的 `ResultView.test.tsx / SimulationView.test.tsx` 共 **12 passed**；`e2e:full` 与 `e2e:debate:full` 的既有工件仍可复查，但本轮没有把 `npm run build` 继续写成通过项 |
