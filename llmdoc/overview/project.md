...
| Oracle Chambers / 世界线圆桌 | 当前口径是“前端单结局可玩版 + 后端 Phase B+ 基座”：结果页每张结局卡当前会先经过 participant picker，再进入 `进入会客厅 / 只改一步`；前端 `EndingChatModal + endingRoomStore + useEndingRoomWS` 已接入 follow-up thread、`archivist_route / hotseat / all_present`、room/thread scope notice、participant 卡与 Oracle 专属 UI 资产；后端除独立 `ending_room` 域、REST/WS、branch-scoped transcript isolation 外，本 session 还补齐了 `selected_agent_ids / follow-up thread / user-turn / room-thread memory partition / worldline echo / all_present`。这轮真实签收仍只覆盖单结局桌面/移动端闭环；`世界线圆桌` 页面、多结局完整独立签收，以及 ending-room replay/share/import 仍未完成，所以这条线还不能按完整玩法签收 |
...
| Automation QA Hooks | 浏览器自动化钩子 — `render_game_to_text()` 覆盖关键页面，Theater 暴露 `advanceTime(ms)`；InputView 现在会输出 `page.daily_challenge / page.weekly_challenge / page.director_growth`，Simulation 页面输出 `page.replay_state / replay_source / warmup / director / betting`，ResultView 输出 `archive_summary / result_bet_list / result_key_moments / result_branch_snapshots`，DebateResultView 输出 `replay_source` 与 `result.adjudication_mode`。前端提供 `e2e:matrix / e2e:variants / e2e:corners / e2e:cross-browser / e2e:safari / e2e:full` 与 `e2e:debate:*` 入口；主模式 `mobile` 现通过 `scripts/e2e-suite.mjs mobile` 进入签收链路。`corners` 当前又新增 deterministic `live_fork_marker` fixture：固定 scenario API 与专属 fake WebSocket 会把页面推进到“分叉已发生 + completed replay-ready”，然后直接在浏览器里断言 `R1 · fork 1` marker 可见、`R2 · fork 1` 不存在，专门锁住 live `fork_round` 回归。`e2e-suite.mjs` 里的 `director_state_roundtrip / gameplay_state_roundtrip` 当前也已在 PUT 前先回读最新 authority revision，再发起写入，避免固定历史样本因 revision 漂移稳定撞 `409`；`scripts/e2e-automation.mjs` 的主模式 `predict` 流本 session 也已把等待条件改成“URL 或 automation 先进入 simulation，再等真实可点击的预测入口”，降低 SPA / hydration 抖动下的假超时；share 生成与 share retry 的等待当前也已跟前端更长的社交文案超时窗口对齐，减少正常 LLM 延迟下的假失败。`release:signoff` 会落盘带 git `branch / commit / worktree` 绑定信息的 `summary.json`，并把 backend targeted `pytest`、`/metrics`、`tsc / build / perf budgets / assets / corners / mobile / cross-browser / debate:full` 组成默认签收合同；在显式要求时，Debate signoff 还会校验 `adjudication_mode`。CI 当前除了 `debate-signoff-smoke` 之外，还新增了 `release-signoff-fixture`：无 secrets 时，主模式会走隔离 mock LLM（默认 `18318`），Debate 强约束 `deterministic`，不会占用真实 LLM 路径。仓库内最近一次通过的 clean real full signoff 工件仍见 `frontend/output/e2e/post-commit-signoff-clean/summary.json`（绑定 commit `421f6d6c37980a5fb1b79cf6ddd664f5ecda3473`，status=`passed`，`dirty=false`，且要求 `adjudication_mode = llm_hybrid`）；本 session 又实跑通过 `frontend/output/e2e/20260325-session-release-signoff/summary.json`（status=`passed`，dirty=`true`，绑定当前 worktree）；当前 checkout 是否已严格签收，仍应以最新实跑 `summary.json` 为准。 |
