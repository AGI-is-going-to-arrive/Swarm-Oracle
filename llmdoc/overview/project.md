...
| On-Demand Theater Loading | Theater 现在只会在用户真正切进 Pixel Theater 后，按 idle 时机预热 Phaser；截图链路里的 `html2canvas / gif.js / gif.worker` 也都保留在截图/GIF 路径上按需加载，避免首页和 Classic 路径提前拉取这批大依赖 |
