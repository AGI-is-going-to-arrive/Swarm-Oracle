...
| Automation QA Hooks | 浏览器自动化钩子 — `render_game_to_text()` 覆盖关键页面，Theater 暴露 `advanceTime(ms)`；Simulation 页面新增 `page.replay_state`、`page.warmup`、`page.controls.capture_mode/can_capture_modal/capture_result_kind`，并通过 `capture_game_screenshot(panel|canvas|modal)` 提供显式截图模式；Track D 的 Debate live/result 现额外输出 `selected_phase / is_phase_locked / unlocked_phases / active_modal / modal_state / bet_window_open` 等状态摘要；前端同时提供 `e2e:matrix / e2e:variants / e2e:corners / e2e:full` 与 `e2e:debate:*` 一键回归入口，`e2e-suite.mjs` / `e2e-debate-suite.mjs` 都会在输出目录落盘 `browser-launch.json`，记录本次实际采用的浏览器启动 profile；`capture-modes` 现在会额外落盘 `predictionModalBytes / gameplayModalBytes`，`replay` corner case 会等到 `theater_ready = true` 再判定恢复完成；`e2e-suite.mjs` 的截图链路现已补 `timeout + CDP fallback`，`history-delete-last-page` 会在最终截图前等待删除弹窗真正脱离 DOM；`e2e-debate-suite.mjs` 现已支持 `--width / --height / --question / --profile-hint` 覆盖。Track C 主基线工件位于 `frontend/output/e2e/20260317-track-c/{matrix,corners,mobile,variants}/`；本轮 full smoke 工件位于 `frontend/output/e2e/20260318-codex-full-smoke/result.json`，结果页 backend summary smoke 工件位于 `frontend/output/e2e/20260318-result-backend-summary-smoke-v2/result-final.json`；Track D Debate 当前可复查工件包括 `frontend/output/e2e/20260318-post-b2-debate-full/`、`frontend/output/e2e/20260318-debate-mobile-430x932-v2/` 与 `frontend/output/e2e/20260318-debate-civic-v2/` |
...
| Generated Gameplay Art | 使用 Gemini 图像模型生成并接入玩法卡专属 frame、Theater 场景背景、徽章和因果档案装饰面板；Track D 本轮另生成了辩论专用背景、stage banner、verdict panel、score meter、三方 badge 与 quote frame；这批 Debate 新 PNG 已补 `.meta.json` provenance，仓库里更早的一部分 generated 资产仍在补齐队列 |
...
| Open Source Polish (Phase 4) | 开源准备 — Weather 粒子对象池 + 视口裁剪 + `ASSET_CREDITS` 现已同步到 Debate Arena 新资产；当前新生成的 Debate 资产已补同名 `.meta.json`（`preset/model/provider/source/source_url/generated_at/output/prompt`），其余旧 generated 资产仍在逐步补齐 |
| i18n | 中英文支持 |
...
| 工具 | 覆盖 |
|------|------|
| pytest + pytest-asyncio | 本轮已验证后端全量回归 **815 passed**；最近定向回归包括 `test_card_events.py / test_gameplay_contract_sync.py` 共 **24 passed**、`test_predictions.py` 共 **17 passed**，以及这次重新验证的 `test_config.py / test_campaign_api.py / test_campaign_service.py` 共 **14 passed**；其中 shared contract、`scenario summary` 新接口与默认 DB/Chroma 路径锚定都已重新验证 |
| Vitest + jsdom | 本轮已验证前端全量回归 **179 passed**，覆盖状态管理、回放、截图、玩法卡、场景推断和页面自动化摘要；最近已真实执行的定向回归为 `gameplayContract / gameplayCards / GameplayCardsModal / scenarioMeta` 共 **26 passed**，以及这次文档同步重新执行的 `DebateArenaView.test.tsx / ResultView.test.tsx / SimulationView.test.tsx` 共 **14 passed**；`npm run build` 也已恢复通过；`e2e:full` 与 `e2e:debate:full` 的既有工件仍可复查 |
