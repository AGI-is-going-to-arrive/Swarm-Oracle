...
| Oracle Chambers / 世界线圆桌 | 当前口径是“前端可玩签收版 + 后端 Phase B+ 基座”：结果页每张结局卡会先经过 participant picker，再进入 `进入会客厅 / 只改一步`；前端 `EndingChatModal + endingRoomStore + useEndingRoomWS` 已接入 follow-up thread、`archivist_route / hotseat / all_present`、room/thread scope notice、participant 卡与 Oracle 专属 UI 资产；`WorldlineRoundtableView + worldlineRoundtableStore + useWorldlineRoundtableWS` 现已落地 live roundtable、`改选代表并重开`、`archivist_route / hotseat` 追问与只读 replay。后端除独立 `ending_room` 域、REST/WS、branch-scoped transcript isolation 外，也已补齐 `selected_agent_ids / selected_representatives / follow-up thread / user-turn / room-thread memory partition / worldline echo / all_present`，并把 Oracle 关键文案链路升级成 `LLM-first + deterministic fallback`，同时补了 room-stable `user participant` 与 SQLite `database is locked` 短重试。最新稳定 `release:signoff` 工件 `frontend/output/e2e/20260330-full-release-signoff-stable/summary.json` 中，`ending_room_followup / roundtable_full / debate_full` 均为 `passed`；后续在 `8317` 真模型链路下也已实测通过 ending-room / roundtable Oracle 专项验证。当前未完全签收的残余主要集中在 follow-up 与经典模式的流式一致性、roundtable mobile 一屏体验，以及 roundtable 代表/档案官声线还可继续去模板化 |
