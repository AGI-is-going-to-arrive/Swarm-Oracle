...
| Automation QA Hooks | 浏览器自动化钩子 — `render_game_to_text()` 覆盖关键页面，Theater 暴露 `advanceTime(ms)`；Simulation 页面现输出 `page.replay_state`、`page.replay_source`、`page.warmup`、`page.controls.capture_mode/can_capture_modal/capture_result_kind`、`page.controls.can_copy_replay_link`、`page.director.objectives / system_tracks / commitment`，以及新的 `page.betting`；ResultView 的自动化摘要现还会输出 `replay_source`、`archive_summary`（含 `objective_completed_count / objective_total_count / commitment_outcome / risk_value / resource_value`）、`result_bet_list / result_key_moments / result_branch_snapshots`；DebateResultView 也会输出 `replay_source` 与 `result.adjudication_mode`；前端继续提供 `e2e:matrix / e2e:variants / e2e:corners / e2e:cross-browser / e2e:safari / e2e:full` 与 `e2e:debate:*` 一键回归入口，`e2e-suite.mjs` / `e2e-debate-suite.mjs` 都会在输出目录落盘 `browser-launch.json`；当前 `frontend/package.json` 还新增了 `release:signoff`，会顺序跑 `tsc / build / corners / cross-browser / debate:full`，Safari 可按需附加。本 session 已实跑 `frontend/output/e2e/2026-03-19T15-20-32-479Z-release-signoff/`、`frontend/output/e2e/2026-03-19T15-25-24-398Z-release-signoff/`（含 Safari）与 `frontend/output/e2e/2026-03-19T15-35-24-820Z-release-signoff/`；更早的 cross-device corners 工件仍在 `frontend/output/e2e/20260319-cross-device-state-corners/`、`frontend/output/e2e/20260319-cross-device-state-cross-browser/`、`frontend/output/e2e/20260319-cross-device-state-safari/`；若要补 Safari panel capture 取证，可看 `frontend/output/e2e/20260319-cross-device-state-safari-panel/` |
