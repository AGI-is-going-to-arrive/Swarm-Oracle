...
- 独立 `/workbench/:id` 图谱工作台保留 `view` 与 `branch` query，三种视图继续按各自 capability gate 判断；结果页 header 和下一步 bridge 都会带上当前 analysis branch，页面顶部会提供返回对应结果页的链接。graph tab 会把 URL 里的 `branch` 继续传给 causal graph 请求，不会退回默认全图。
...
- 最近完整本地验证：
  ...
  - frontend full vitest：`202 files / 2203 tests passed`
  - frontend i18n key + placeholder parity：`zh: 2761`、`en: 2761`、parity OK
  - CausalGraphBoard workbench browser spot-check：用户给出的短 `branch=c7c0f252` 在本地返回 `404 BRANCH_NOT_FOUND`；同一 scenario 的完整 branch id 页面通过 zoom label density、keyboard focus tooltip、node click、ExportPanel、语言切换、reduced-motion 和 390px mobile overflow 复核
