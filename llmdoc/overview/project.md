...
| On-Demand Theater Loading | Theater 现在只会在用户真正切进 Pixel Theater 后，按 idle 时机预热 Phaser；本 session 又把 `PhaserGameLoader` 本身移到 Theater 动态边界，并把截图链路拆成 `useScreenCapture` 轻壳 + `screenCaptureRuntime` 按需加载，`html2canvas / gif.js / gif.worker` 继续只在截图/GIF 路径上加载 |
