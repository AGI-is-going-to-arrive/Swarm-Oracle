...
- 当前口径：
  ...
  - token 仍保留为 fallback
  - `scenario_result_v1` 当前会先压缩 replay 用 `scenarioMeta`：只保留 replay 页真正需要的 archive 输入，不再把整包结果摘要副本写进 payload
