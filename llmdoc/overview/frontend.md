...
- `BootScene.ts`、`VizSynthesizer.ts` 与 `gameplayCards.ts` 已共用这份 registry，避免扩新主题时多处手工同步；当前 `BootScene` 只预载首屏初始 theme + 角色 sprite，后续 `scene_change` 背景与 `EndingScene` 大图改为按需补载，`bet_panel / leaderboard / title_screen / minimap_frame` 也不再是 Theater 首次进入硬依赖
...
- **测试**:
  ...
  - Current targeted frontend set: **100 passed**
