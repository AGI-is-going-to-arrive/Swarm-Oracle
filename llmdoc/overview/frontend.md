...
| DebateArenaView | `DebateArenaView.tsx/.css` | Debate live 页：首页 `Debate Arena` 入口会创建独立 debate，会场首屏展示辩题、阶段条、势能条、三方 score card、阶段发言和结果 CTA；当前先用 DOM 舞台复用 Theater 场景图与 Debate 专属 UI 资产，不把 Debate 硬塞进 Phaser 场景；live 页现已支持阶段锁定回看、`Arena Read / 局势解读` 面板、移动端固定底部主 CTA，并会按 `debate.language` 自动同步整页 UI 语言；当前移动端同屏只保留一个主 CTA，避免 hero CTA 与底部 rail 重叠；页面已接 `render_game_to_text()` / `advanceTime(ms)` / `capture_game_screenshot()`，方便桌面/移动端自动化取证；当前 live 页会优先读取后端显式 `counterplay` snapshot/WS 数据，本地 helper 只做兜底；首轮 `counterplay` 同步现在还会忽略与当前路由 `debate id` 不一致的 stale snapshot，避免上一场辩论的反制记录短暂闪进新局；本轮又补了赛况总览卡、房间席位图、阶段地图，以及后端 `phase_insights` 直出的阶段解读；`debate_verdict` 到来时，这批阶段洞察会直接进 store，而不是等结果页二次拉取；live 页下注与 counterplay 失败提示当前也已改成按 `ApiError.code` 映射本地化文案，手动下注失败也会显示同口径 notice；首页三档主模式 `神谕档位 / Oracle Profile` 在这里当前只会显示为“不适用主模式神谕档位”的提示，不会伪装成 Debate 自己的运行时配置 |
