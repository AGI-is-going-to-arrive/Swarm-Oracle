...
- **Actions**: `startSimulation(options)`，当前与 `api/client.ts#createScenario()` 共用同一份 `CreateScenarioOptions` 口径（`question / rounds / numAgents / mode / hierarchical / llmApiKey / llmBaseUrl / llmModel / reasoningEffort / temperature / branchSensitivity / forkPromptVariant / forkDetectorActiveBranchLimit / visualizationEnabled / userId`），不再在页面层手工透传长位置参数
