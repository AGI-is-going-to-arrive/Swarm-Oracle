...
| 页面 | 文件 | 描述 |
|------|------|------|
| InputView | `InputView.tsx/.css` | 首页：输入"What-If"问题，Agent 数量与轮数滑块当前口径是 `min=3 / max=40`，默认都为 `5`；滑块下方会显式显示 `最小 / 最大` 刻度，避免把默认值误看成“上限没生效”。推演模式切换(RAW/Blackboard)、BYOK 自定义 LLM 折叠面板 (P4-E) 和快速开始卡片仍保留；问题输入在移动端使用自动增高多行 textarea，长问题会换行而非裁切；每日挑战卡支持中英文文案与完成态/已用卡数/下注态反馈，并会显示当前题材 label、signature hooks 与题材回响反馈；today/weekly challenge 定义当前优先来自后端 `GET /api/campaign/challenges/rotation`，前端不再维护本地 challenge catalog；若 rotation 不可用，首页也不再本地合成 today/weekly challenge 内容；首页会同时读取 director campaign `profile / mastery / badges / daily-status / weekly-summary`，把后端真值与本地缓存合并显示；当前还补了 `weekly challenge` 与 `director growth` 两张 Lite 卡，以及从分享 challenge URL 预填 `question / rounds / agents / mode / viz / profile / preset` 的 banner；首页现在只读取轻量题材摘要 helper（`label / hooks / badge`），不再直接 runtime 引完整玩法策略表；BYOK/provider policy 当前会优先持久化到 `sessionStorage`，供 `createScenario / createDebate / social copy / scorePredictions` 共用，并会在首次读取时把旧 `localStorage` 记录迁移一次后清掉；BYOK 面板当前除了既有 `API Key / Base URL / Model`，也正式显示 `RPM / TPM` 输入，每项字段下方都补了一句人话说明；`Test Connection` 现在不只测连通性，还会在成功后显示 provider `probe` 摘要、推荐的 `agents / rounds` 区间，以及当前 endpoint 是否允许 `disable user quota` 覆盖；若用户填了 BYOK API key 但当前没有 fresh probe，`Start Simulation` 会先自动做这次预检；当用户填了 `RPM / TPM` 时，首页当前还会即时给出预算提醒卡，并在当前 `agents / rounds` 超预算时直接禁止主模式 `Start Simulation`；首页里的 BYOK 测试错误、启动错误与 automation 摘要当前都已收口到同一套结构化错误映射，签收时看到的不再是原始后端报错字符串；页面内部当前也已按 `useInputByokSettings / useInputCampaignState / useSharedChallengePrefill` 三组 hook 收边界，提交主链与 `createScenario` 现统一走 options 对象；除了既有 `llmApiKey / llmBaseUrl / llmModel / reasoningEffort / llmRequestsPerMinute / llmTokensPerMinute`，首页当前还正式暴露主模式 `神谕档位 / Oracle Profile` 三档（Watchful/Calibrated/Riftbound；中文守望/校准/裂界），把 `branchSensitivity / forkPromptVariant / forkDetectorActiveBranchLimit` 映射到 `createScenario`；当前也会按 `agents × rounds` 显示主模式预计耗时，而首页底部 `3-5 分钟` 提示已明确只指 Debate Arena |
| DebateArenaView | `DebateArenaView.tsx/.css` | Debate live 页：首页 `Debate Arena` 入口会创建独立 debate，会场首屏展示辩题、阶段条、势能条、三方 score card、阶段发言和结果 CTA；当前先用 DOM 舞台复用 Theater 场景图与 Debate 专属 UI 资产，不把 Debate 硬塞进 Phaser 场景；live 页现已支持阶段锁定回看、`Arena Read / 局势解读` 面板、移动端固定底部主 CTA，并会按 `debate.language` 自动同步整页 UI 语言；当前移动端同屏只保留一个主 CTA，避免 hero CTA 与底部 rail 重叠；页面已接 `render_game_to_text()` / `advanceTime(ms)` / `capture_game_screenshot()`，方便桌面/移动端自动化取证；当前 live 页会优先读取后端显式 `counterplay` snapshot/WS 数据，本地 helper 只做兜底；首轮 `counterplay` 同步现在还会忽略与当前路由 `debate id` 不一致的 stale snapshot，避免上一场辩论的反制记录短暂闪进新局；下注 modal 与 quick counterplay 当前都会严格消费后端 `available_prediction_options`，不再靠前端硬编码目标；若 room 在 modal 打开后已经跨进 `closing / verdict`，页面也会按后端返回的锁单错误显示本地化提示；本轮又补了赛况总览卡、房间席位图、阶段地图，以及后端 `phase_insights` 直出的阶段解读；`debate_verdict` 到来时，这批阶段洞察会直接进 store，而不是等结果页二次拉取；live 页下注与 counterplay 失败提示当前也已改成按 `ApiError.code` 映射本地化文案，手动下注失败也会显示同口径 notice；首页三档主模式 `神谕档位 / Oracle Profile` 在这里当前只会显示为“不适用主模式神谕档位”的提示，不会伪装成 Debate 自己的运行时配置；390px 左右窄屏下，阶段 chip 也已再压一轮宽度，`Opening / Crossfire / Rebuttal` 首屏不会再明显裁切 |
| DebateResultView | `DebateResultView.tsx/.css` | Debate 结果页：展示 winner、scoreline、维度 breakdown、best argument / best rebuttal / judge summary、结构化 `judge_rationale`、`supporting_turns`、phase replay digest、prediction settlement 与 share modal；顶部 hero、verdict panel、quote frame、side badge 等都已接入 Debate D4 资产；结果页会按 payload `language` 自动同步 UI 语言，若 result API 返回终态错误会直接展示，不再无限轮询；分享弹窗按钮与生成文案首行现会按平台带 emoji 图标；当前结果页会优先读取后端显式 `counterplay` 字段，展示反制摘要、命中/未中结果、`explanation`，并继续传给 share copy；share copy 现还会带 1-2 条关键引文；本轮又补了结果信号卡与阶段地图，并优先消费后端顶层 `phase_insights`，所以 live / result 的阶段 stakes、judge focus 和 commentary 现在是同一套口径；同一组件当前还负责 `/debate/replay/result`，会从 replay token 直接 hydrate 结果、显示 `adjudication_mode`，并支持“导入为本地运行”；若 replay 是通过 `import-replay` 导入的，结果页当前会优先展示那份已持久化的 `phase_insights`，避免把原始阶段解读重算成近似版本；结果页 automation 当前也会把错误字段收成 `{code, label}`，和 UI 口径一致；与 live 页一样，首页三档主模式 `神谕档位 / Oracle Profile` 在这里当前也只保留“不适用主模式神谕档位”的提示 |
| SimulationView | `SimulationView.tsx/.css` | 模拟进行中：实时agent对话流 + 干预入口 + 可收起侧边栏（磨砂玻璃药丸Toggle） + 分支卡片实时讨论动态脉冲；当前 `InterventionModal` 已支持 `即时 / 回溯 / 批量` 三模式，页面会把活动分支、分支历史轮次上限和当前轮次传给 modal；Theater 顶部包含 live status HUD、玩法卡入口、结构化下注入口、显式截图模式切换（`Panel/Canvas/Modal`）以及“当前截取目标”提示；完成态支持世界线/轮次回放筛选、重播/跳到最新/倍速控制，并把 compact TimelineBar 直接嵌回 Theater 面板内；live warmup 会显示“导演准备中”状态，玩法卡支持只读预览；当前导演层会在 Theater 面板内显示 2 个短目标、常驻 `风险 / 资源` 轨道、worldline 承诺选择与锁定/取消按钮；这部分 goals / commitment 现在走后端 `director_state` 权威态，主模式 `cards.usageLog / betting.bets / archive.key_moments / archive.branch_snapshots` 都走后端 `gameplay_state` 权威态，本地 `scenarioMeta` 退为缓存/兼容层；结构化下注提交成功后会立刻把新 meta 回传父层并回写后端；authority 写入链路当前已补齐 `revision` 版乐观并发控制：页面写入会带上当前 revision，若命中 `409`，会先回读最新 `director_state / gameplay_state` 再继续，并给出可见反馈；本 session 又去掉了把后端 authority 反向 apply 到 localStorage 的镜像链路，页面内只做 authority 优先的内存合并与 replay payload 组装；标准干预与玩法卡注入成功后，页面当前都会显示队列反馈：没有排队时提示“预计下一轮生效”，有积压时会显示前方待处理条数；玩法卡 modal 现会优先消费后端返回的 `gameplay_state`，不再只靠本地 usage 推导来刷新导演点数与冷却；玩法卡 modal 现会显示题材专属三段式连锁事件、`风险 / 资源` 两条轻量状态，以及 profile-specific 的打法说明；live `branch_fork` 当前会直接消费后端事件里的真实 `children[*].fork_round`，所以 Theater 时间线和 completed replay-ready round marker 不会再把新子分支临时记成 `R0`；这条修复只影响分叉轮次展示，不改变回放轮次可用集合，也不影响主模式 `神谕档位 / Oracle Profile` 的三档映射；尾声阶段只要进入 `narrating`，或 final-round tail 已经跑完、thinking agents 清空但后端还没来得及切 `done`，页面当前都会按 `600ms` 主动补拉 scenario，而 store 不再允许迟到的 `simulating` 把 `done` 覆盖回去；导演承诺在锁定 / 清除时当前还会显示“保存中 / 已锁定 / 已清除”的短反馈；桌面端 Theater 本轮又做了“场景优先”的可读性修复：右栏收成固定窄宽、`game-wrapper` 提前到 replay filters/timeline 前、导演目标改成双列、compact timeline 隐藏二级 stats；live/replay 气泡节奏和截断策略也已收口，窄屏只保留单气泡并把 agent 名称并进正文，Agent > 12 时改成前后双排半弧站位；移动端 Theater 当前会把顶部动作区、状态条、导演摘要和 replay filters 收成单行横向滚动条带，本轮又继续压缩了 card/chip/button padding，并把主剧场高度再让出更多首屏空间；页面切进 Theater 时也会重新收起 agent panel，避免隐藏侧栏继续占高把首屏主剧场顶出视口；视图切换仍收成图标胶囊，避免控件裁切；Theater 背景现为 30 个 Theater 语义主题 + 3 个 Debate 专属主题，共 33 个 registry 条目；当前 Phaser 预热只会在用户真正切进 Theater 后再触发，并优先走 idle 时机；Classic 视图下只有在页面可见、设备允许且用户 hover/focus 到 Theater toggle 时，才会轻量预取 loader chunk；当前默认前端 build / vitest 也已通过 alias 指向本地精简 Phaser 入口 `experiments/phaser-custom/entry.mjs`；`BootScene` 当前只预载首屏初始 theme 与 `sprite_default + 初始角色 sprite`，其余缺失 sprite 改由 `WorldScene` 在 agent 真到场时按需补载，后续 `scene_change` 背景与 `EndingScene` 大图继续按需补载；截图自动化摘要现还会带 `capture_result_kind`，便于区分 `gif` 与 `gif_fallback_png`；同一组件当前还负责 `/sim/replay`，replay 模式会关闭 WS、下注、干预和玩法卡写操作，只保留回放 / 截图 / 复制 replay 链接 / 导入为本地运行；页头当前只显示一个主模式 `神谕档位 / Oracle Profile` 标签，不再把 prompt variant / 世界线灵敏度 / detector 预算拆成一排技术 chip；标签值优先按 `scenario.fork_debug.round_checks` 识别，识别不到时才回退当前会话 preset；页面 automation 当前也会把这份 `runtime_preset` 摘要和 `error` 字段一起输出成稳定结构，便于签收时确认当前主模式到底是 `Watchful / Calibrated / Riftbound` 哪一档；页面内部当前已按两组 hook 收边界：`useSimulationReplayState` 负责 replay hydrate 与回放筛选，`useSimulationViewState` 负责截图与 director/gameplay authority 状态；replay / director / theater scene-weather-time 文案也已统一进 i18n 词条，不再在页面里手写中英分支 |
| ResultView | `ResultView.tsx/.css` | 结果页：分支树 + 叙事对比 + 社交媒体文案生成 (P6)；如果用户在 narration 完成前过早打开 `/result/:id`，页面会先停在 loading，等故事真正生成完再展示；故事与档案摘要就绪后会调用 `finalizeCampaign()` 结算导演生涯，并渲染 `Campaign Progress` 区块（积分增量 / 等级 / 下次解锁 / 徽章）；结构化下注与“因果档案”区块现已支持按单条下注显示 `命中 / 未中 / 待判定`，档案摘要会展示命中比，并保留 `dominantBranchTitle`、`dominantTone`、`mostUsedCard`、`archiveGrade`、`directorStyleTag`、`profileResonance`；本轮新增的归档字段包括导演目标完成度、worldline 承诺结果，以及最终 `风险 / 资源` 轨道；结果页现在会把远端 `betting / archive` raw state 合并回展示层，`key moments` 会按当前 UI 语言格式化展示，并新增 `branch snapshots` 区块；页面当前会先读后端 `director_state`、`gameplay_state`，再并行读取后端 `/api/campaign/scenario/{id}/summary` 回填 `archive_grade / profile_resonance / most_used_card / betting_hit / completed_daily_challenge`，并避免把默认 `3/3` 导演点数误当成真值展示；当前结果页对 authority 已进一步收口：远端 `director_state / gameplay_state / campaign summary` 会作为展示基线，只有缺字段时才回退本地 `scenarioMeta` 兼容内容；如果远端只显式带某个 gameplay 分区，也只替换那一块，不会把其它本地兼容字段顺手清空；结果页当前会先用 `story.branches / usages / bets / commitment / campaign summary` 现算 `displayArchive / displayBranchSnapshots`，再把本地 `scenarioMeta.archive` 降成最后兜底；若同一标签页会话里已经拿到同一 `scenario + director + profile` 的 finalize 完整结果，结果页二次进入时当前会复用缓存而不再重复 `POST /finalize`，但首次进入仍会拿完整 `profile / mastery / badges` 数据；页头当前只保留一个主模式 `神谕档位 / Oracle Profile` 标签，不再展开一排 prompt / sensitivity / budget 技术细节；移动端结果页的顶部操作区当前已改成单列堆叠，`导出 Markdown / 生成文案 / 复制固定链接 / 分享挑战 / 查看排行榜` 不再把按钮挤出视口；`分享挑战` 按钮当前会把同题同参数连同 `preset` 一起编码回首页预填；导出 Markdown 当前已改成浏览器直连后端附件下载，不再走本地 `blob:` 拼接链路，目标是让 Chrome / Edge 都稳定下载 `.md` 文件；主模式 `ShareModal` 当前也已把冗余 context chips 收掉，并停止给社交文案自动追加题材 / 神谕 / 固定链接前缀，只保留平台正文；`scorePredictions()` 当前也会读取同一份 session-scoped BYOK/provider policy，把 `llmApiKey / llmBaseUrl / llmModel` 透传到评分请求；对应回归测试当前已覆盖“会话内 provider overrides 生效”和“评分失败会显示错误”；结果页当前已把 Oracle Chambers 结果卡入口和 `WorldlineRoundtableView` 主入口都接进稳定签收链：每张结局卡会先打开 participant picker，再进入 `进入会客厅 / 只改一步 / 异线旁听席`；modal 通过 `endingRoomStore + useEndingRoomWS` 消费后端 `ending_room` snapshot/result，并在 room 可能快于 WS 建连完成时主动补拉一次 snapshot/result，收掉“房间已 done 但前端永久停在 draft”的竞态；`发起圆桌` 当前会进入独立 `WorldlineRoundtableView`，支持 live roundtable、`改选代表并重开`、`archivist_route / hotseat` 追问、只读 replay，以及 `manual_shortlist / expert_witness` 扩展桌型；fresh dev session 下前端当前主连 `/api/ws/ending-room/{room_id}`，backend 仍保留 `/ws/ending-room/{room_id}` alias；这条 Oracle 链路当前已通过稳定 `release:signoff` 总链，但 ending-room 的 replay/share/import 单链仍未单独签收；`/result/replay` 当前也会优先依赖 replay payload 里的 `storyData` 与精简版 `scenarioMeta`，并在读路径里把 usage-derived 状态补回运行态，不再把整包 archive 摘要副本带进 replay；同一组件当前还负责 `/result/replay`，会优先从后端短 `share id` 读取 replay，失败时回退 token，并跳过 `finalizeCampaign()` 与后端 authority 回写，必要时可直接“导入为本地运行”；若 replay 导入失败，页面当前会保留结果页并显示 inline error，而不是静默恢复按钮状态 |
| WorldlineRoundtableView | `WorldlineRoundtableView.tsx/.css` | 世界线圆桌页：按多结局结果创建 live roundtable，支持每条结局 1 名代表入桌、`改选代表并重开`、`archivist_route / hotseat` 追问、只读 replay、本地导入，以及 `render_game_to_text() / advanceTime(ms)` 自动化钩子；当前 picker 已支持 `representative / manual_shortlist / expert_witness` 三种桌型，后两者都已形成最小可玩闭环；页面 automation 现还会输出 `selection_mode / selected_branch_count / has_witness` 等稳定 state，并已进入稳定 `release:signoff` 总链；移动端当前已继续压缩 live-room 首屏，transcript 更早进入视口，但真实口径仍是“可滚动可用”，不是“一屏签收” |
| HistoryView | `HistoryView.tsx/.css` | 历史列表页：分页查看 scenario 记录、按状态筛选并支持删除；列表加载失败时会按当前 UI 语言重建错误文案，不再在切语言后继续显示旧语种提示 |
| LeaderboardView | `LeaderboardView.tsx/.css` | 排行榜页：展示预测排行榜与 Top-50 条目；列表加载失败时会按当前 UI 语言重建错误文案，不再在切语言后继续显示旧语种提示 |
...
| `EndingChatModal` | Oracle Chambers 单结局会客厅 modal；当前支持 `进入会客厅 / 只改一步 / 异线旁听席` 三种结果页入口、draft/commit transcript 收口、thread rail、`New thread`、只读 replay 提示，以及 automation 输出 `ending_room_modal` 状态摘要；当前 follow-up 已支持 `archivist_route / hotseat / all_present` 三种模式，会按 room/thread scope 展示 committed transcript，并已补上当前 responder / 当前追问对象 / 模式说明这类更像玩法而不是系统说明的前端提示；`crossline_gallery` 当前也已直接复用这套 modal 壳，收口为只读摘要旁听视图；这条线现已通过单结局桌面/移动端闭环，以及多结局结果页按各自 ending 打开 chamber / one-move / gallery 的专项 E2E；当前未单独签收的残余主要是 follow-up 与经典模式的流式一致性 |
...
| `DebateBetModal` | Debate Arena 结构化押注弹窗，只支持 `winner / verdict_tone`；现会输出 modal 内部 automation state，并显示更直白的押注说明文案；当 live 页触发 `counterplay` 预设时，它会接收 preset kind/target/confidence；当前所有 target 选项都以后端 `available_prediction_options` 为准，不再在前端单独硬编码一套候选列表 |
...
- 单结局会客厅当前已支持：
  ...
  - `档案官主持 / 点名角色 / 当前阵容回应` 这一组更直白的人话文案与模式说明
...
- 当前真实签收边界：
  - 单结局桌面 / 移动端闭环：通过
  - 多结局结果页按各自 ending 打开 picker / chamber / one-move / gallery：通过专项 E2E
  - `WorldlineRoundtableView` 的 live / reseat / follow-up / replay readonly：已进入稳定 `release:signoff` 总链并通过
  - `manual_shortlist / expert_witness`：已形成最小可玩闭环，并通过专项 view test + full E2E
  - 当前仍未单独签收的残余：replay/share/import 最终专项、follow-up 与经典模式的流式一致性、roundtable mobile 一屏体验，以及 `trait_mix / fault_line_first / witness_augmented`
...
- 当前安全读接口已补有限瞬态错误重试：`GET` 路径会对 `429 / 500 / 502 / 503 / 504` 做小步退避重试；写接口仍保持“不自动重试”，避免误放大副作用
- 当前 LLM provider overrides 除了 `llmApiKey / llmBaseUrl / llmModel / reasoningEffort`，也已统一带上 `llmRequestsPerMinute / llmTokensPerMinute`
...
- **Actions**: `startSimulation(options)`，当前与 `api/client.ts#createScenario()` 共用同一份 `CreateScenarioOptions` 口径（`question / rounds / numAgents / mode / hierarchical / llmApiKey / llmBaseUrl / llmModel / llmRequestsPerMinute / llmTokensPerMinute / reasoningEffort / temperature / branchSensitivity / forkPromptVariant / forkDetectorActiveBranchLimit / visualizationEnabled / userId`），不再在页面层手工透传长位置参数
...
### `endingRoomStore.ts` (Zustand)
- **状态**: `snapshot`、`result`、`status`、`error`、`errorCode`、`pendingDrafts`
- **Actions**:
  - `openRoom(scenarioId, payload)` — 创建或复用单结局 ending-room room
  - `loadRoom(roomId)` — 拉取 room snapshot / result
  - `hydrateSnapshot()` / `hydrateResult()` / `setPhase()` / `setStatus()` / `setResult()` / `setError()`
  - `startDraft()` / `appendDraft()` / `commitTurn()` — 处理 `turn_start -> turn_delta -> turn_commit`
- **特点**:
  - 只服务 Oracle Chambers `Phase C` 单结局 modal，不和 `simulationStore` 混用
  - `mergeSnapshot()` 会按 `sequence` 合并 turn，并保留更终态的 `status/current_phase`
  - room 已经在 backend 侧快进到 `done` 时，store 仍可通过后续 `loadRoom()` 一次性补齐 transcript/result
  - `status=error` 时也统一走 `errorCode + error`，不把原始异常字符串直接漏给页面
...
- 当前口径：
  - `scenarioMeta` 当前已补 per-scenario localStorage 写锁（短租约）与 `_rev` 递增记录；多个标签页对同一 scenario 的常规写入会先做有限次同步抢锁重试，再基于最新快照落盘；若短时间内仍拿不到锁，就会带 warning 退回 optimistic 路径，不再用 busy-wait 卡住主线程，所以能收住大多数跨标签覆盖，但并不承诺彻底强一致
...
- 当前优先级：
  ...
  3. 本地记录
- 本地缓存当前只保留最近约 `30` 天的记录；读取时会顺手清掉过期 `debateId`
...
- 当前口径：
  - `ending_tone / profile_resonance` 的命中判断现已改成规范化精确匹配：会统一做 `lowercase + NFKC + 去标点 + 压缩空白` 后，再和 `targetId / 中文标签 / 英文标签` 做等值比较
  - 也就是说，像 `balanced order` 这类只包含子串的标签，当前不会再误判成命中 `order`

### `runtimePreset.ts`
- 首页主模式 `神谕档位 / Oracle Profile` helper
- 负责：
  - 将 `Watchful / Calibrated / Riftbound`（中文 `守望 / 校准 / 裂界`）映射到主模式 `branchSensitivity / forkPromptVariant / forkDetectorActiveBranchLimit`
  - 把当前选择按会话写入 `sessionStorage`
  - 在 `SimulationView / ResultView` 按 `scenario.fork_debug.round_checks` 回读当前实际生效的档位；若当前页面还拿不到有效 round checks，则回退当前会话 preset
- 当前口径：
  - 这组三档只作用于主模式 worldline branching，不影响 Debate Arena
  - 其中 `Calibrated / 校准` 仍是默认推荐档
...
- 本地 progress store 当前也会按最近约 `30` 天做懒清理；旧日期桶会在读取时自动收口，不再无限累积
...
- 首页状态拆分 helper，当前导出三组 hook：
  - `useInputByokSettings()`：负责 provider policy 读写、BYOK 表单状态和连通性测试；当前会优先读写 `sessionStorage`，并在首次 hydrate 时把 legacy `localStorage` provider policy 迁移一次；provider policy 当前已正式带上 `disableUserQuota / requestsPerMinute / tokensPerMinute`；同时也维护 `probeResult / hasFreshProbe / disableUserQuota`，让首页在 fresh probe 失效时先重跑 BYOK 预检，再决定是否发起 `createScenario`
...
- 当前口径：
  ...
  - 当前 `RPM / TPM` 的预算提醒与主模式启动阻断也收在这组 BYOK 状态里，不再让页面层各自再算一遍
  - 主模式 `神谕档位 / Oracle Profile` 当前不走这组 hook，而是由 `runtimePreset.ts` 单独管理会话持久化与 URL 预填
...
- 当前口径：
  ...
  - 命中 `409` 时，hook 当前会标记冲突类型，交给页面显示“已回拉最新承诺/玩法状态”的轻提示
...
- event id 去重当前已从 `string[] + includes()` 收口为 bounded `Map`；重连 debug 也会先记录真实 `reconnectCount` 再清零
...
- Debate hook 这轮也已同步改成 bounded `Map` 去重，并修正了 `onopen` 里 `reconnectCount` 的记录时序
...
### `useEndingRoomWS.ts`
- WebSocket 当前主连到 `/api/ws/ending-room/{id}`；backend 仍保留 `/ws/ending-room/{id}` alias 兼容旧链路
- 分发最小 ending-room 事件：
  - `heartbeat`
  - `status`
...
- 当前也会消费非 heartbeat 事件顶层 `meta`，在 hook 边界做：
  - `event_id` 去重
  - stale sequence 丢弃
  - gap 触发的 `getEndingRoom() / getEndingRoomResult()` resync
- `onopen` 当前会主动做一次 resync，而不是只在 reconnect 时补拉；这是为了解决 ending-room backend 可能在 WS 建连前就已 `done` 的竞态
...
`WSEvent` 为联合类型，覆盖 15 种WebSocket事件（含 `retrospective_start`、`batch_intervention_applied`、`simulation_error`）；除 `heartbeat` 外，当前还可带可选顶层 `meta`（`stream_id / sequence / event_id / manager_instance_id / emitted_at`）。其中 `branch_fork.children[*]` 当前会显式带 `fork_round`，供 live store 和 completed replay-ready 时间线直接读取真实分叉轮次。
...
- 一键回归入口：
  ...
  - `scripts/fork-experiment.mjs` 当前会批量创建 scenario、轮询完成态、收集 `fork_debug.round_checks` 与 `/story`，并同时输出 `result.json + results.csv + samples/*.json`；实验维度现已支持 `temperature / branch_sensitivity / fork_prompt_variant / fork_detector_active_branch_limit / rounds / num_agents / concurrency`
  - `capture-modes` case 现在会额外落盘 `predictionModalBytes / gameplayModalBytes`
  - `corners` 当前又新增了 deterministic `live_fork_marker` fixture：通过固定 scenario API + 专属 fake WebSocket 顺序推送 `agent_speak / round_summary / branch_fork / simulation_done`，直接在浏览器里断言 `.timeline-round[title="R1 · fork 1"]` 可见、`R2 · fork 1` 不存在
...
- `frontend/public/fork-control-room.html` 当前提供一个独立静态控制室页面：用非 React 路径把 fork detector 的控制变量、真实实验结果和产品默认配置建议可视化，便于直接分享给非开发同事或在浏览器里快速对照
...
| Structured Betting 2.x | `PredictionModal` 支持押世界线 / 押结局倾向 / 押题材回响，Theater HUD 可直接打开下注；当前会先展示 bet kind / 目标 / 当前轮次 / 当前承诺等下注摘要；分支尚未 hydrate 或当前没有可下注活跃世界线时，会自动回退到 `ending_tone`，避免空目标下注；下注提交成功后会立刻把新 meta 回传父层，并由 `SimulationView` 直接回写后端 `gameplay_state` |
...
| Automation Hooks | `render_game_to_text()` + `advanceTime(ms)` + `preserveDrawingBuffer=true`，提升浏览器黑盒测试的状态读取与截图稳定性；已覆盖页面级控件摘要、warmup 摘要、capture mode、modal 内部状态摘要与分享上下文摘要；当前 automation payload 里的错误字段也已统一成 `{code, label}`，和 UI 口径一致；`scripts/e2e-automation.mjs` 的主模式 `predict` 流当前也已补成更稳的双重等待：先等 `route-or-automation` 进入 simulation，再等真实可点击的预测按钮，减少 dev / hydration 抖动下的假超时 |
...
- 本 session 这轮“BYOK sessionStorage / WS meta dedup+gap resync / wsDebug / ResultView 评分链路”改动，当前还额外实跑通过：
  ...
  - `npx tsc --noEmit -p tsconfig.app.json`：通过
- 本 session 这轮“首页神谕档位正式上架 / 分享 challenge 带 preset / 结果页单标签收口 / 主模式 ShareModal 去前缀 / Markdown 附件下载”改动，当前还额外实跑通过：
  - `src/pages/InputView.test.tsx + src/i18n/locales.test.ts`：`14 passed`
  - `src/lib/shareEnvelope.test.ts + src/components/ShareModal.test.tsx + src/pages/ResultView.test.tsx`：`22 passed`
  - `npx tsc --noEmit -p tsconfig.app.json`：通过
  - Playwright 实测主模式 `corners`：`frontend/output/e2e/20260324-runtime-preset-corners/`
  - Playwright 实测 Debate `full`：`frontend/output/e2e/20260324-runtime-preset-debate/`
  - Playwright 实测结果页导出下载事件：建议文件名 `swarmoracle-ad179268.md`
- 本 session 这类“scenarioMeta 跨标签同步 / replay 校验 / InputView challenge rotation / WorldScene ambient mote pool / GameplayCardsModal 状态收敛”改动，当前还额外实跑通过：
  ...
  - `src/lib/scenarioMeta.test.ts + src/pages/SimulationView.test.tsx + src/pages/ResultView.test.tsx + src/lib/scenarioReplay.test.ts`：`47 passed`
    ...
    - `npx tsc --noEmit -p tsconfig.app.json`：通过
- 本 session 这轮“scenarioMeta 锁竞争去 busy-wait / Debate stale counterplay 守卫 / replay 导入错误提示 / History+Leaderboard 语言切换错误文案”改动，当前还额外实跑通过：
  - `src/lib/scenarioMeta.test.ts + src/pages/DebateArenaView.test.tsx + src/pages/ResultView.test.tsx`：`41 passed in 1.80s`
  - `src/pages/HistoryView.test.tsx + src/pages/LeaderboardView.test.tsx`：`2 passed in 0.496s`
  - `npx tsc --noEmit -p tsconfig.app.json`：通过
- **发布签收**:
