...
| GraphEdge evidence 深水区 | `GraphEdge` 已有 `confidence_tier / source_ref / source_round_number / evidence_json` 字段，causal graph 新边也会返回 evidence；但旧边不自动回填，debate argument-map API 仍未返回 evidence，`evidence_json` 也还没有真实填充来源 |
