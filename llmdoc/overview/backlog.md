...
| GraphEdge evidence contract 尚未落库 | `GraphEdge` 还没有稳定的 `confidence_tier / source_ref / source_round_number / evidence` 字段；Graphify 证据层如果进入实现，需要先确认迁移、API response 和前端消费口径，不能把 `payload_json` 当稳定 contract |
