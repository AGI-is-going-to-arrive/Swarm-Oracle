...
- In each round an Agent can perform one native social action: `POST`, `COMMENT`, `REACTION`, `FOLLOW`, `MUTE`, `SEARCH`, `TREND`, `REFRESH`, or `IDLE`. Follow and mute operate on currently visible accounts; a muted source no longer appears in that Agent's Feed, search, or trend results.
