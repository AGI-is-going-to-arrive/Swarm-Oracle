...
- **The run stays on "Preparing" or looks idle?** A local LLM can take a while to produce round 1, so the top progress ledger shows elapsed time; after that, content frames or in-round progress keep the run in the still-working state. If the background task really was interrupted, the backend reconciles orphaned runs on startup or polling, and the frontend keeps the interrupted-run panel visible so you can return home and retry with the original question.
