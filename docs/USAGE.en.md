...
2. Set Agent count, rounds, and mode. To seed the social environment, add up to 20 events in the initial world-event Feed: provide a source and content, then optionally add publication time, a credibility hint, and tags. You can also load and edit the built-in template. Feed content is untrusted data; do not paste credentials or treat it as a system instruction. Use Advanced Settings for Classic / Pixel Theater, search augmentation, and related options. Open BYOK separately when this run needs another LLM connection. When you use a saved profile unchanged, result pages recover it from the scenario; changing its endpoint or model clears the old profile's model, key, and RPM/TPM, so enter a complete new binding (an exact-local endpoint may still omit the key).
...
- In each round an Agent can perform one native social action: `POST`, `COMMENT`, `REACTION`, `FOLLOW`, `MUTE`, `SEARCH`, `TREND`, `REFRESH`, or `IDLE`. Targets, search, trends, and refresh open only when the previous round's real opportunity snapshot allows them; an action bound to a domain rule must also match the same frozen schema, state revision, and threshold. Follow and mute operate on currently visible accounts; a muted source no longer appears in that Agent's Feed, search, or trend results.
- The live domain-state strip shows up to six bounded variables for the current branch, their latest verified changes, and units. A variable's threshold entry lists met and unmet rules with actual/expected values. When the latest complete round chooses `IDLE` because a domain precondition is blocked, the strip separately names the Agent and blocked rules. A missing valid schema, legacy data, or replay failure is `unavailable`; the UI does not calculate state from utterances.
...
- Inspect the domain-state strip and World Outcomes. Outcomes summarize initial → final values and net changes per branch only when deterministic replay ties them to verified durable actions, with bounded action, rule, and available Claim refs. No verified change, partial, and unavailable remain distinct; an absent change is not written as an outcome.
...
- **Graphs**: `/workbench/:id` combines Causal, Split, and Knowledge Graph; `/kg-explorer/:id` and `/timeline-galaxy/:id` provide standalone views. The Action Ledger panel in Causal Review shows persisted native actions, targets, states, and domain adjudications for the effective branch lineage; expand an action to inspect its rule, before/after values, and failure code. A separate evidence-ledger API projects utterances, context observations, and derived consequences, with memory represented only by hashed references and source-scenario coordinates. Older runs without the corresponding evidence show `unavailable` instead of inferring it from prose. Legacy causal `stance` nodes and edges are displayed only as affect proxies, not verified stance or causal evidence.
...
- **Why does an older run lack complete Action Ledger evidence?** Native actions and context receipts exist only for runs that recorded the corresponding contracts. Older data is `unavailable`; the UI does not guess actions or memory use from utterance text.
- **Why can I not find a verified-memory-promotion switch or record?** It is currently a default-off backend core that becomes effective only when both `FEATURE_AGENT_IDENTITY=true` and `FEATURE_MEMORY_PROMOTION=true`, with no capability, REST, or UI entry. Do not treat existing Identity Inspector or Action Ledger hash refs as proof that promotion is active; the Stage 3B activation surface is not delivered.
