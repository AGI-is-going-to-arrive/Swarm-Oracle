...
- **Bounded domain world**: the parser may ask the model to propose variables and rules, but a fixed code contract freezes only a `domain_world_v1` schema that passes type, unit, precision, bounds, action-binding, and credential checks. Complete rounds deterministically adjudicate domain changes from durable verified actions only. The live state strip shows current values, latest changes, threshold tooltips, and domain-gated IDLE attribution. A missing valid schema, legacy data, or an unreplayable branch is explicitly `unavailable`.
...
- **Terminal worldlines**: show verdict, confidence, per-leaf answers, probabilities, and stories. Likelihood and analytic confidence count only completed terminal leaves as branch samples and use evidence counts; fork parents do not count, and signed affect-convergence proxies do not raise analytic confidence. When a domain schema exists, the result also lists each branch's verified state changes with bounded action/rule/claim source coordinates, keeping no-change, partial, and unavailable states distinct.
...
- **Branch comparison**: inspect speech, transitions, and domain-variable differences between the original and new branches, including the first verifiable domain divergence's round, rules, and action sources. An invalid side is labeled unavailable.
...
- **Action Ledger**: the Causal Review panel shows persisted native actions, targets, states, and domain adjudications for the current scenario and effective branch lineage. The same deterministic domain history drives the state strip, threshold tooltips, IDLE attribution, and world outcomes. A separate evidence-ledger API projects Agent utterances, context observations, and derived consequences. Memory evidence exposes only hashed references and source-scenario coordinates, not memory text. Older data without the corresponding evidence is `unavailable` rather than inferred.
...
- **Identity, memory, and growth**: persistent identity profiles show persona, knowledge domains, decision style, memory, and growth events. Simulation memory is isolated by Agent, branch lineage, and round scope. A separate default-off backend verified-memory core becomes effective only when both `FEATURE_AGENT_IDENTITY=true` and `FEATURE_MEMORY_PROMOTION=true`; it then promotes only results exactly bound to a verified action, domain adjudication, and actual delta, using a versioned namespace and stable hashed refs for later recall. It has no capability, REST, or UI entry today, so it cannot be enabled or inspected from the interface.
...
- **Replayable Feed, actions, domain state, and runtime**: initial Feed entries, source accounts, native actions, the frozen domain schema, Decision Envelopes, and State Transitions are preserved in Snapshots. Import remaps and revalidates branch, Agent, message, and action coordinates, then rebuilds domain state and transitions from durable actions. Replay restores the same action and runtime history within the effective lineage and cutoff, while branch Compare includes structured transitions in its diff when they exist. A counterfactual replacement does not fabricate a new decision: it marks the replaced coordinate unavailable and requires later replanning. Legacy data without runtime or a domain schema remains `unavailable` instead of inferring actions, decisions, or state changes from prose. Memory-promotion refs carried by Snapshot/import/clone remain opaque history and are not resolved into recallable sources.
...
Feature entries vary with flags, data state, and replay/live mode. Capability checks distinguish loading, error, disabled, and enabled states; failures offer retry instead of being reported as disabled. If an older snapshot lacks observation provenance, structured runtime, or a domain schema, the UI labels it as a snapshot, no matching observation, or `unavailable` instead of inventing a worldline, round, decision, or state change. Local Pack context is cleared when switching to another entry such as Quick Start, so it cannot leak into the next launch. Gallery and Agent Pack are local, offline file workflows rather than hosted registries, online publishing, or community indexes. Other missing fields degrade to read-only, partial, or unavailable states. Decision Envelopes, State Transitions, DomainWorld, and Claims are auditable records inside a simulation, not real-world measurements or causal proof; `memory_write_candidates` remain candidates, and the default-off promotion core is not a delivered UI. All probabilities, relationships, and statements are simulated and are for entertainment and exploration.
