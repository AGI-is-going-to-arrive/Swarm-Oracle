...
- **干预与玩法卡**：只在 live 推演中可写；叙事、完成态和 replay 为只读。
...
- **复盘**：因果档案、导演复盘、用户预测对比、run-group 分布和阵营时间线。
- **导出**：Markdown、Snapshot、固定链接、预测卡片和脱敏公开 artifact。
...
- **身份与记忆**：档案展示 persona、记忆和成长事件；自定义 Agent 可加入主推演和辩论。
...
- **Snapshot 演示**：无真实 LLM 时也能导入 `samples/snapshots/*.swarm`，查看保存的结果和 replay。
...
功能入口会随 feature flags、数据状态和 replay/live 模式变化。旧 Snapshot 可能缺少新字段；页面会降级为只读、partial 或 unavailable，不会伪造结果。所有生成内容只供娱乐和探索。
