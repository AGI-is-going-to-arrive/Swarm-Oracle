...
### F01 首页提问框与开始推演
在首页输入一个「如果……会怎样？」的问题，然后启动主推演。你会先确认问题和本局设置，再进入多 Agent 推演。输入法组词期间不会误开确认弹窗；慢请求或重复点击会保持单一启动，并在启动链路超时后解除锁定。问题为空、LLM 未配置或本轮预算被挡住时，开始按钮下方会显示原因。
入口：`/` 首页 -> 输入问题 -> `开始推演`
![首页提问框](screenshots/01-home.png)

### F02 辩论竞技场
首页可以直接进入辩论竞技场。它会创建正方、反方和评委，按固定阶段推进一局更短的对抗讨论。无可用 LLM 凭据或启动失败时，页面会显示原因而不是静默停住。
...
推演模式提供谨慎、均衡、探索三档，只影响主模式的世界线分岔取向。高级设置还可以在 Classic 与 Pixel Theater 之间切换显示；Pixel Theater 用像素舞台展示角色发言、分支和导演工具栏。推演页顶部会用分段进度账条显示当前轮次、轮内 Agent 发言进度、整体进度和预计剩余时间；多分支并行时，发言进度会标出当前分支。首轮还在预热时会显示已运行时长，本地模型较慢但仍有内容、进度活动或有效运行信号时会提示仍在工作。底部进度条和 Theater 回放时间线会避开缩放控件、消息列表和语言切换器，并兼顾移动端安全区。长时间没有任何内容、进度活动或有效运行信号时，页面会显示中断面板，可回首页重新发起并保留原问题；真正失败、取消或后台任务确实中断时，未完成分支会显示为已中断而不是推演中或低概率剪枝。干预入口只在 live 推演中可用，进入叙事、完成或 replay 后会自动隐藏。导演/玩法卡来自共享契约，当前 14 张为：文明辩论、间谍渗透、密约交易、人类潜入、时空裂缝、民意浪潮、撤离令、公开听证、资源分诊、禁术仪式、审计清算、情报反噬、民意回摆、停火委员会。Debate Arena 有自己的回合规则，不跟随这个设置。
入口：`/` 首页 -> `推演模式` / 显示模式；运行中 `/sim/:id` -> Pixel Theater toolbar -> 玩法卡
![推演模式](screenshots/01-home.png)

### F05 推演轮数与 Agent 数量
两个滑块控制主推演的长度和参与角色数量。页面会按当前选择估算等待时间，方便你在速度和细节之间取舍；推演开始后，顶部进度账条会继续显示当前轮次、轮内发言进度、整体进度、预计剩余时间和叙事生成状态，首轮未产出时也会用已运行时长说明系统仍在等待模型返回。
入口：`/` 首页 -> `推演轮数` / `Agent 数量`
...
### F08 世界线卡片
每条结局都有标题、概率、故事摘要和对原问题的回答。概率按终局叶子世界线归一展示，父分叉不会被当成最终结局；合法的单一结局仍可显示为 100%。展开卡片后可以继续阅读完整故事或进入后续入口。
入口：`/result/:id` -> 世界线卡片
...
### F38 搜索增强推演
**需配置，见 [CONFIGURATION](CONFIGURATION.md)。** 打开后，系统会在推演前搜索资料，并把相关片段注入角色提示。推荐路径优先使用服务端已配置搜索；高级路径允许本轮使用自己的 provider。结果页会把普通网页片段、来源家族结果和模型原生 citations 汇总到同一个“真实世界来源”信息流。
入口：`/` 首页 -> `搜索增强推演`
![搜索增强推演](screenshots/01-home.png)

### F39 来源家族复选框
**需配置，见 [CONFIGURATION](CONFIGURATION.md)。** 开启 `FEATURE_NEW_SOURCES` 后，首页高级设置会显示预测市场、财经资讯、学术文献和深度报道四类来源。它们只有在搜索增强已打开、且 provider 支持域名过滤时才会生效；结果页会把四类来源作为可筛选行汇入统一来源信息流，并保留概率、引用数、发布时间、历史记录和可解释状态。
入口：`/` 首页 -> 搜索增强 -> 来源复选框
...
入口：`/` 首页 -> `高级设置` -> `文档种子上下文`（拖拽或选择 PDF / TXT / Markdown 文件）

### F48 本地主题包
**默认开启，见 [CONFIGURATION](CONFIGURATION.md)。** 首页「高级设置」折叠区里的「本地主题包」收录了一批预设场景（中英双语，含提示词、押注点和素材）。选择器现在按类型分段和单一搜索框筛选，标签文本并入搜索；当前结果会自动选中并在右侧预览，筛选状态会显示结果数和清除入口。点一下模板就能把场景的问题和推荐设置填入提问框，适合快速上手或课堂演示。主题包放在仓库的 `packs/` 目录，可以自己增删。
入口：`/` 首页 -> 高级设置 -> 本地主题包 -> 选包 / 使用此模板
