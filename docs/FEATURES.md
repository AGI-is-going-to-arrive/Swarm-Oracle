...
推演模式提供谨慎、均衡、探索三档，只影响主模式的世界线分岔取向。高级设置还可以在 Classic 与 Pixel Theater 之间切换显示；Pixel Theater 用像素舞台展示角色发言、分支和导演工具栏。推演页顶部会用分段进度账条显示当前轮次、进度和预计剩余时间；首轮还在预热时会显示已运行时长，本地模型较慢时改为仍在工作的提示。底部进度条和 Theater 回放时间线会避开缩放控件、消息列表和语言切换器，并兼顾移动端安全区。长时间没有轮次进展时，页面会显示中断面板，可回首页重新发起并保留原问题；干预入口只在 live 推演中可用，进入叙事、完成或 replay 后会自动隐藏。导演/玩法卡来自共享契约，当前 14 张为：文明辩论、间谍渗透、密约交易、人类潜入、时空裂缝、民意浪潮、撤离令、公开听证、资源分诊、禁忌仪式、审计清算、情报反噬、授权回摆、停火委员会。Debate Arena 有自己的回合规则，不跟随这个设置。
入口：`/` 首页 -> `推演模式` / 显示模式；运行中 `/sim/:id` -> Pixel Theater toolbar -> 玩法卡
![推演模式](screenshots/01-home.png)

### F05 推演轮数与 Agent 数量
两个滑块控制主推演的长度和参与角色数量。页面会按当前选择估算等待时间，方便你在速度和细节之间取舍；推演开始后，顶部进度账条会继续显示当前轮次、整体进度、预计剩余时间和叙事生成状态，首轮未产出时也会用已运行时长说明系统仍在等待模型返回。
入口：`/` 首页 -> `推演轮数` / `Agent 数量`
