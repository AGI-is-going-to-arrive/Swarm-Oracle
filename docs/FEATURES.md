...
这份图鉴覆盖发布前核对的 41 个用户可见功能。入口和默认开关以当前代码、使用指南和配置说明为准；搜索相关的三项功能默认关闭，需要先完成配置。
...
### F04 推演模式
推演模式提供谨慎、均衡、探索三档，只影响主模式的世界线分岔取向。Debate Arena 有自己的回合规则，不跟随这个设置。
入口：`/` 首页 -> `推演模式`
...
### F41 结果深读报告
**默认开启，见 [CONFIGURATION](CONFIGURATION.md)。** 结果页会显示深读报告入口，按章节汇总关键结论、证据坐标和不确定性。报告也可以在 `/result/:id/report` 独立查看；生成或重试走 `report:generate`，失败时不会阻断原本的结果页。
