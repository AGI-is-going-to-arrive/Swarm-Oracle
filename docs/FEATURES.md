...
### F48 本地主题包
**默认开启，见 [CONFIGURATION](CONFIGURATION.md)。** 首页的「本地主题包」收录了一批预设场景（中英双语，含提示词、押注点和素材）。选择器现在按类型分段和单一搜索框筛选，标签文本并入搜索；当前结果会自动选中并在右侧预览，筛选状态会显示结果数和清除入口。点一下模板就能把场景的问题和推荐设置填入提问框，适合快速上手或课堂演示。主题包放在仓库的 `packs/` 目录，可以自己增删。
入口：`/` 首页 -> 本地主题包 -> 选包 / 管理全部
