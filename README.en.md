...
- **Multi-branch simulation**: One question, multiple storylines; the simulation page shows current round, progress, ETA, and first-round warmup elapsed time with a slow-model hint, interrupted runs become retryable, and the result page reports normalized probabilities for terminal leaf worldlines only.
