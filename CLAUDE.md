...
- Source family 分类搜索：`FEATURE_NEW_SOURCES` 控制，开启后 InputView 展示 4 个领域 checkbox（预测市场/财经资讯/学术文献/深度报道）；当前 checkbox 还受 `/api/capabilities.web_search.provider_capability.supports_domain_filter` 门控，不支持 domain filter 时保持可见但 disabled，并会清空旧选择。Tavily/Exa 走 API 域名过滤，SearXNG 走归一化 `site:` 查询，xAI app-layer 走 Responses `tools[].filters.allowed_domains`；所有 family 结果都会再做 URL 后过滤。`provider_capability` 目前描述服务端默认 provider，不是当前用户 custom override 的实时探测结果。
- InputView 模式选择器（推演模式/可视化模式/推理力度/神谕档位）默认展示在输入区域下方，不再折叠在"高级设置"中；BYOK（自定义 LLM）保留独立折叠区
- InputView textarea 有 IME 组合输入三重防护（`isComposingRef` + `nativeEvent.isComposing` + `keyCode===229`），中文/日文/韩文输入法按 Enter 确认候选词不会触发推演；Enter/按钮提交改为先弹出确认弹窗（问题预览+设置摘要），再确认后才真正调用 `launchSimulation`
- 辩论竞技场论证图谱（ArgumentMap）在 `DebateArenaView` 中全宽展示在页面最底部（不在侧栏内），默认展开；DAG 容器高度 `min(75vh, 800px)`
- 辩论 turn 卡片（正方席/反方席/裁决席）统一为 `debate-turn-card` 样式：4px 左边框 side-color（正方品红 `--debate-accent-pro` / 反方蓝 `--debate-accent-con` / 评委铜 `--debate-accent-judge`）、首字母彩色头像（`.debate-speaker-initial`）、衬线标题（Cormorant Garamond）、16px 圆角卡片；FoldableTurn 在辩论上下文折叠高度 `--foldable-collapsed-height: 120px`，正文 `line-clamp-2` 多行截断；所有 `oklch()` / `color-mix()` 声明都有 sRGB fallback + `@supports` 渐进增强
- 辩论 phase insights（`stakes`/`judge_focus`/`commentary`/`strategy`）在辩论结束时由 `_enhance_insights_with_llm()` 用 LLM 重写，受 `DEBATE_USE_LLM` gate，失败回退到模板文本
- 辩论 persona 去模板化：`debate_prompts.py` 新增 `generate_persona_with_llm()` + `build_cast_async()` 异步生成辩论角色人设（3 方并行，失败回退到模板），LLM persona 持久化在 `breakdown_json.metadata.personas`，`_serialize_debate` 通过 `_persona_for()` 优先读取 LLM persona、回退模板；persona 在 system prompt 中经 `format_untrusted_text_block()` 包装为 UNTRUSTED DATA 防注入
- 辩论 prompt 去模板化：`phase_argument_goal`/`_side_specific_instructions`/output requirements 全部重写为口语化自然语言，system message 含禁用术语列表（「机制」「执行后果」「责任链」等），turn 生成有 pass-2 retry（温度 0.8→0.6）后 fallback 到 `anchor_copy`
- `GET /api/capabilities` 是轻量配置端点（无 LLM 调用），前端 mount 时用它检测服务端功能开关；`web_search.provider_capability` 只暴露 domain-filter/source 能力，不包含 key、base URL 或内部 secret
...
- 2 个新 API router：`/api/agents/*`、`/api/graphs/*`；`/api/capabilities` 当前是 `web_search` + 16 个功能开关的 capability registry
...
- Oracle 文案去模板化：`_content.py` 新增 3-tier fallback 架构（generation-first temp=0.82 → rewrite temp=0.78 → static anchor），`_build_oracle_generation_prompt` 以角色身份+场景问题+对话摘录驱动生成，不再依赖 anchor copy；`_build_character_identity_block` 整块通过 `format_untrusted_text_block` 注入；`_oracle_vocabulary_prompt_section` 把可信的静态 voice variant 词汇和 persona snapshot 身份提示分开，动态身份提示只进入 `UNTRUSTED DATA`；`_build_factual_guardrail` 从 branch_card 提取轻量事实（worldline_title/key_hinge/outcome_insight/key_moments）替代完整模板文本；四种房间类型（WORLDLINE_ROUNDTABLE/ENDING_CHAMBER/ONE_MOVE_ONLY/CROSSLINE_GALLERY）均已接入 factual_guardrail + scenario_question + transcript_quotes 三项上下文注入
- `_load_scenario_question` + `_load_branch_transcript_excerpts`：分别加载场景原始问题和分支最近 5 条对话摘录；transcript loader 当前用窗口函数取每条分支 top-N，并支持 `branch_ids` 过滤，follow-up 只加载本次需要的分支摘录
- `_stream_oracle_copy` 改用 `_build_oracle_generation_prompt`（不再走 rewrite prompt），温度 0.55→0.75，reasoning_effort low→medium；如果 stream 结束后没有任何可见内容（空 chunk 或仅 reasoning block），会退回非流式改写
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-05-13 | Web Search / Source Family P0-P1 contract hardening | 后端：新增 provider capability registry，`native` 保持 legacy placeholder 且 preflight 改为 warn；base search 与 family search 拆成独立 try-block，base 失败不阻断 family；xAI app-layer source family 改用 Responses `filters.allowed_domains`（每次最多 5 个域名并标记 partial coverage）+ URL 后过滤，SearXNG 先归一化域名再拼 `site:`，family URL 过滤要求 `http/https`、IDN/punycode 归一化，并用 exact/subdomain 匹配防 suffix lookalike；`family_context` parser 保留扩展状态和 `domain_filter_mode / domain_coverage / status_reason` 元数据。前端：InputView 根据 `provider_capability.supports_domain_filter` 禁用 Source Family checkbox，关闭 web search 或 capability 不支持时清空已选 family，提交 payload 只在 web search 开启且支持 domain filter 时带 family；i18n 补齐禁用提示。限制：`provider_capability` 目前描述服务端默认 provider，不是 custom override 的实时探测；ResultView 还没有为所有扩展状态补专门文案。真实验证：backend full `2835 passed / 2 skipped`，backend Web Search 定向 `249 passed` + URL scheme 窄集 `28 passed`，`ruff check .` 通过；frontend full vitest `184 files / 2004 passed`，InputView/API 定向 `76 passed`，tsc/lint/build 通过；Playwright 复核桌面 1440、移动 375 与 Chromium/Firefox/WebKit headless smoke |
| 2026-05-07 | SwarmOracle 去模板化补录 + worker synthesis 加固 | 后端：补录 `memory.py` persona 指令去模板化、`_content.py` anchor copy 简化、Oracle generation/rewrite 双路径上下文透传与防注入边界；移除 `_followup_angle_label` / `_oracle_role_pressure_clause` 两个已无调用的旧模板 helper 及 re-export；`_build_character_identity_block` 改为整块 `format_untrusted_text_block`，persona vocabulary 的动态身份层也改成 `UNTRUSTED DATA`；Tier 2 `_build_oracle_rewrite_prompt` 同步接收并注入 `scenario_question` + `transcript_quotes`；`_stream_oracle_copy` 对空流式 / 仅 reasoning 输出退回非流式 rewrite；`_load_branch_transcript_excerpts` 改成窗口函数 + `branch_ids` 过滤。`simulator.py` 的 hierarchical worker synthesis 明确依赖 `_stable_pick` / `_extract_meaningful_fragment` / `_synthesize_worker_response`，新增直接单元覆盖确定性、空输入、单选项、Unicode/CJK 句界、软/硬截断与中英文 silent/template 分支。前端：圆桌 helper/caller/i18n 迁移补录，`describeRoundtableAnchor` 签名对齐为可选 `TFunction`，新增 phase 类型 guard，移除生产 `as unknown`/`as never` cast；补齐 en/zh placeholder parity，并把脆弱源码字符串断言改成真实行为断言。真实验证：backend `2399 passed / 2 skipped`，frontend `165 files / 1782 passed`，tsc 0 errors，ruff 0 errors，i18n key/placeholder parity `1587=1587` |
| 2026-05-07 | 辩论竞技场全面去模板化 | 后端：`debate_prompts.py` 新增 `generate_persona_with_llm()` 异步 LLM persona 生成（temp=0.85，JSON 输出，失败回退模板）+ `build_cast_async()` 3 方并行生成；`phase_argument_goal`/`_side_specific_instructions`/output requirements 全部重写为口语化自然语言；system message 新增禁用术语列表；persona 在 system prompt 中从 `sanitize_untrusted_text` 升级为 `format_untrusted_text_block` 防注入。`debate.py`：`DebateRuntimeSnapshot` 改为 mutable + 新增 `personas` 字段；`run_debate_background` 新增 async persona upgrade 步骤（持久化到 `breakdown_json.metadata.personas`）；`_serialize_debate` 新增 `_persona_for()` 优先读 LLM persona、回退模板；`_finalize_debate` 通过 `_extract_persisted_personas_meta()` 合并 personas 到新 metadata；turn 生成新增 pass-2 retry（temp 0.8→0.6）；judge analysis prompt 移除禁用术语；`_build_phase_stakes/judge_focus/commentary` 简化为中性短句。前端：`DebateArenaView.tsx` 修复 judge note 在 result_ready 时仍显示模板文本；`DebateScoreCard` + `DebateArenaView` 新增 persona 展示；`types.ts` 新增 `strategy`/`persona` 字段；i18n 简化 6 条模板化文案。Codex+Claude 双模型对抗式审查修复 1 Critical（persona 防注入升级）+ 3 Warning（judge 术语/aria-label/变量遮蔽）+ 7 回归测试；135 debate tests passed，tsc 0 errors，ruff 0 errors，vite build 0 violations；后续修复全部预存测试失败后 fresh baseline: backend `2386 passed / 0 failed / 2 skipped`，frontend `165 files / 1769 passed / 0 failed` |
| 2026-05-07 | 辩论竞技场 turn 卡片排版重构 | 前端：`FoldableTurn.tsx` 折叠状态从 `text-ellipsis whitespace-nowrap` 改为 `line-clamp-2` 多行截断，解决中文字符在 44px maxHeight 下被像素级截断显示乱码的问题；`DebateArena.css` 全面重构（+346 行）：新增 OKLCH 色彩 token（`--debate-accent-pro/con/judge`）+ sRGB fallback + `@supports` 渐进增强、room grid 从 3 列改为 1 列堆叠、FoldableTurn 辩论覆盖（`article.group` 提升 padding/border-radius/font-size 对齐 debate-turn-card 质感）、speaker 首字母头像（`.debate-speaker-initial--{side}`）、面板标题改衬线字体、accordion `border-b` 覆盖（`!important` 压制 Tailwind utility）、卡片入场动画 `debateCardIn` + `prefers-reduced-motion` guard、situation grid 改 `auto-fit` 响应式；`DebateArenaView.tsx` SpotlightTurnCard 替换为 `debate-turn-card` article（统一所有 turn 卡片样式）、speaker 首字母头像圈、side-specific phase chip（`.debate-phase-chip--con/--judge`）、spotlight 展开/折叠 toggle + `useEffect` 随 turn 切换自动重置。Codex 对抗式审查修复 1 Critical（CSS fallback）+ 3 Warning（spotlight 状态生命周期、collapsed 高度不足、judge avatar 对比度）；Playwright 桌面 1440px + 移动端 375px 截图验证通过；fresh baseline: frontend `165 files / 1763 passed / 6 failed`（6 为预存 InputView），tsc 0 new errors，vite build 0 violations |
| 2026-05-07 | 首页 IME 防护 + 推演确认弹窗 + 辩论竞技场 UI/UX 重构 | 前端：`InputView.tsx` 新增 IME 组合输入三重防护（`isComposingRef` + `nativeEvent.isComposing` + `keyCode===229`），textarea 添加 `onCompositionStart/End` 处理器，解决中文拼音按 Enter 误触发推演的问题；新增推演确认弹窗（`requestLaunch` → 显示问题预览+设置摘要 → `confirmLaunch`），支持 Escape 关闭、backdrop 点击关闭、autoFocus 确认按钮、移动端 column-reverse 响应式、`prefers-reduced-motion` guard；`InputView.css` 新增 `.confirm-launch-*` 弹窗样式（毛玻璃背景 + 圆角卡片 + 入场动画）。辩论竞技场：`DebateArena.css` 修复发言文字不换行溢出（`overflow-wrap: break-word` + `white-space: pre-wrap`）、底部三卡片从 3 列改为单列横条紧凑布局（去除 `min-height: 128px`）、正反方/评委左边框色彩标识（正方粉 / 反方蓝 / 评委金）、accordion 从灰色 border-b 改为圆角卡片式样；`DebateArenaView.tsx` 论证图谱从侧栏移到全宽底部展示 + 默认展开；`ArgumentMap.tsx` DAG 高度增大到 `min(75vh, 800px)` minHeight 480px；`FoldableTurn.tsx` 折叠状态加 `text-ellipsis whitespace-nowrap`、展开状态加 `whitespace-pre-wrap break-words`。后端：`debate.py` 新增 `_enhance_insights_with_llm()` async 函数，辩论结束时用 LLM 重写每个阶段的 commentary（motion + 最近发言 + 分数态势为上下文，`DEBATE_USE_LLM` gate + fallback 到模板），增强后写回 `breakdown_json.metadata.phase_insights`。i18n +2 keys（`confirm_launch_title`/`confirm_launch_settings`），en/zh parity `1638=1638`；fresh baseline: frontend `165 files / 1763 passed / 6 failed`（6 为预存问题），backend `2366 passed / 9 failed`（9 为预存问题），tsc 0 errors，ruff 0 errors，vite build 0 violations |
| 2026-05-06 | 首页 UX 重构 + source family 分类搜索 | 前端：InputView 模式选择器（推演/可视化/推理/档位）从折叠"高级设置"移到输入区默认展示，新增 `.iv-mode-selectors` 卡片容器 + 分隔线排版；web search toggle 去除 `VITE_ENABLE_WEB_SEARCH` 编译期 gate 改为默认可见；source family checkboxes 新增 capability gate（`FEATURE_NEW_SOURCES=false` 时隐藏）+ `.new-source-toggle` 卡片样式（与 web-search-toggle 视觉一致）；BYOK 区独立折叠（去除 `showByok` 二层嵌套）。后端：`web_context.py` 新增 `FAMILY_DOMAIN_FILTERS` 定义四个 family 的域名白名单（polymarket→Polymarket/Metaculus 等，finance→Bloomberg/Reuters 等，academic→arXiv/Scholar 等，news_deep→AP/BBC/NYT 等）；新增 `fetch_family_context()` 异步函数，对每个选中 family 并发发起独立域名过滤搜索（`asyncio.gather`）；四个 provider 函数均新增 `include_domains` 参数（Tavily/Exa 原生 API 参数，SearXNG 通过 `site:` query 语法，xAI 当时通过 prompt 注入；2026-05-13 已改为 `filters.allowed_domains`）；`scenarios.py` 调用 `fetch_family_context()` 替代旧的 `build_source_family_context()` 同步投影；`.env` 新增 `FEATURE_NEW_SOURCES=true`。i18n：source family 标签更新为诚实描述域名搜索范围（如"搜索 Bloomberg、Reuters、CNBC 等财经网站"），en/zh parity `1636=1636`；tsc 0 errors，vite build 0 violations |
