...
- `graphTokens.ts` 新增 `EVIDENCE_TIER_COLORS` 共享常量（CausalReviewView + ArgumentMap 共用，不再各自定义）
- `lib/graphTraversal.ts` 新增 `traceConnectedPath` 共享 BFS 函数（CausalReviewView + ArgumentMap 共用，消除 ~60 行重复代码）
