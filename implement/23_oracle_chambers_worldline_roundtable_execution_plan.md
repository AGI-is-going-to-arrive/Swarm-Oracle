...
> 文档类型：active archive
> 当前真值：历史设计归档；当前实现口径以 `README.md` / `llmdoc/*` 为准
...
> 状态更新（2026-05-24）：Roundtable Overhaul 已把旧 selection recipe 外面补上显式 `discussion_format / cast_mode`。默认是 `deep_dive / smart_pick`；`quick_review` 缩短轮次；`clash_mode` 走挑战式 opening；`custom` 保留用户逐世界线选定代表。旧 recipe、旧 replay、旧 readonly artifact 仍按 fallback 可读；新字段只允许 `worldline_roundtable` 使用，非 roundtable 带新字段会 422。generation version 已升到 5，首开房间会广播瞬时 `ending_room_planning`，前端在 durable turn/result/error 到达后清掉 planning skeleton。本轮真实验证：backend full `3374 passed, 6 skipped`，frontend full `212 files / 2374 tests passed`，i18n parity `2841 / 2841`，Chromium desktop/mobile、Firefox desktop、WebKit desktop 的 roundtable format/cast E2E 均通过；该浏览器结果基于 deterministic backend，不代表真实 provider 慢流式或 Native Safari/Edge 全矩阵签收。
