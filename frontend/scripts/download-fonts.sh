#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# download-fonts.sh — S0-3 Google Fonts localization
#
# Re-fetches the four font families used by SwarmOracle from Google Fonts
# and regenerates frontend/src/fonts.css + frontend/public/fonts/*.woff2.
#
# Run from repo root or from frontend/. Idempotent: existing files are
# overwritten. Requires Node.js >= 18 (uses global fetch + fs.writeFileSync).
#
# Usage:
...
PUBLIC_FONTS_DIR="$FRONTEND_DIR/public/fonts"
FONTFACE_CSS="$FRONTEND_DIR/src/fonts.css"

mkdir -p "$PUBLIC_FONTS_DIR"

GOOGLE_CSS_URL='https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300..700;1,300..700&family=Instrument+Sans:ital,wght@0,400..700;1,400..700&family=Noto+Sans+SC:wght@400..700&family=Noto+Serif+SC:wght@300..700&display=swap'
...
echo "[download-fonts] target dir: $PUBLIC_FONTS_DIR"
echo "[download-fonts] fontface css: $FONTFACE_CSS"

node --experimental-fetch - <<NODE
const fs = require('fs');
const path = require('path');

const GOOGLE_CSS_URL = ${GOOGLE_CSS_URL@Q};
const USER_AGENT = ${USER_AGENT@Q};
const PUBLIC_FONTS_DIR = ${PUBLIC_FONTS_DIR@Q};
const FONTFACE_CSS = ${FONTFACE_CSS@Q};
...
(async () => {
  ...
  const selected = blocks.filter(b => b.subset === 'latin' || (cjkFamilies.has(b.family) && b.subset === null));
  ...
  for (const b of selected) {
    ...
    if (b.subset === null) {
      ...
      const idx = (chunkCounters[key] = (chunkCounters[key] || 0) + 0); // placeholder
      chunkCounters[key] = (chunkCounters[key] || -1) + 1;
      b.localName = slugFamily(b.family) + '-' + weightSlug(b.weight) + styleSuffix + '-cjk-' + String(chunkCounters[key]).padStart(3, '0') + '.woff2';
    } else {
  ...
  console.log('[download-fonts] downloaded', ok, '/', selected.length, '(', (totalBytes / 1024 / 1024).toFixed(2), 'MB,', elapsed, 's)');
  if (fail > 0) {
  ...
  out.push('   Generated by frontend/scripts/download-fonts.sh');
  out.push('   ' + selected.length + ' woff2 files in /public/fonts/, ' + (totalBytes / 1024 / 1024).toFixed(2) + ' MB on disk.');
