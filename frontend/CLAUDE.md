...
| 图谱 | `g6Layouts.ts`, `graphTokens.ts` (共享图谱色彩 token + `EVIDENCE_TIER_COLORS`), `graphTraversal.ts` (共享 BFS 路径追踪 `traceConnectedPath`，邻接表 O(V+E)；另导出 `PERF_ANIMATION_LIMIT=150` 共享常量), `kgGraphConfig.ts` (KG G6 配置工厂: `buildKgG6Options` + `toKgG6Data` + `computeNodeSize` + `getKGNodeStyle/getKGEdgeStyle`，KGGraphBoard 与 KGExplorerView 共用；`toKgG6Data` mobile + 节点 > 200 时截断并通过 `truncatedFromCount` 把原始数量回传给上层；P6 新增 `selected`/`streaming` 状态 + node `{ enter: 'fade' }` 入场动画), `dagEditorialTokens.ts` (DAG 节点 6 类色系 + confidence tiers（medium `#a16207` WCAG AA）+ card 样式常量，预留未接入 GraphNodeCard) |
