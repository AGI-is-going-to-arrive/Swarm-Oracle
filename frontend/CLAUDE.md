...
| `workbench/KGGraphBoard.tsx` | KG 工作台主体 (搜索 / 类型 chips 过滤 / 缩放控件 / 小地图 / SR fallback table / 移动 200 节点 aria-live 截断提示；P6 新增 `lockHighlight`/`clearHighlight` 用 G6 `setElementState` batch API 替代 JS 层 opacity 操纵) |
| `AnimatedEdge.tsx` | 自定义 React Flow 边 (SVG `<animateMotion>` 粒子，仅 selected + !reducedMotion 时动画；集成 `EdgeLabelTooltip` midpoint pill label，P6) |
| `EdgeLabelTooltip.tsx` | 边标签 tooltip (EdgeLabelRenderer portal, hover 300ms delay + Escape dismiss + role="tooltip" + aria-describedby + reduced-motion guard, P6) |
...
| 图谱 | `g6Layouts.ts`, `graphTokens.ts`, `kgGraphConfig.ts` (KG G6 配置工厂: `buildKgG6Options` + `toKgG6Data` + `computeNodeSize` + `getKGNodeStyle/getKGEdgeStyle`，KGGraphBoard 与 KGExplorerView 共用；`toKgG6Data` mobile + 节点 > 200 时截断并通过 `truncatedFromCount` 把原始数量回传给上层；P6 新增 `selected`/`streaming` 状态 + node `{ enter: 'fade' }` 入场动画), `dagEditorialTokens.ts` (DAG 节点 6 类色系 + confidence tiers + card 样式常量，预留未接入 GraphNodeCard) |
...
- **159 个测试文件** (`.test.ts` / `.test.tsx`) / **1677 tests**
