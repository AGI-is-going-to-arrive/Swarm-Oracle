...
| `workbench/KGGraphBoard.tsx` | KG 工作台主体 (搜索 / 类型 chips 过滤 / 缩放控件 / 小地图 / SR fallback table / 移动 200 节点 aria-live 截断提示；P6 `lockHighlight`/`clearHighlight` 用 G6 `setElementState` batch API；P7 NodeConversationSheet 集成；当前图例默认折叠，toolbar 使用 36px icon buttons，KG 节点色板走 WCAG AA 的浅/深色 token；search/filter 会清掉不可见选中节点；workbench 内按 scenario 拉全量 causal graph) |
...
| 图谱 | `g6Layouts.ts`, `graphTokens.ts` (共享图谱色彩 token + `EVIDENCE_TIER_COLORS`；KG 专用 `KG_NODE_TYPE_FILLS` / `KG_NODE_TYPE_FILLS_DARK` 为 WCAG AA 浅/深色板，选中用陶土赭 `#D27050`，hover 用森林绿，继续与 `NODE_TYPE_COLORS_HEX` DAG 色板分离), `graphTraversal.ts` (共享 BFS 路径追踪 `traceConnectedPath`，邻接表 O(V+E)；另导出 `PERF_ANIMATION_LIMIT=150` 共享常量), `kgGraphConfig.ts` (KG G6 配置工厂: `buildKgG6Options` + `toKgG6Data` + `computeNodeSize` + `getKGNodeStyle/getKGEdgeStyle`，KGGraphBoard 与 KGExplorerView 共用；`toKgG6Data` mobile + 节点 > 200 时截断；支持 NODE_ICONS、平行边偏移和 self-loop；P6 `selected`/`streaming` 状态 + fade 入场动画；P7 新增 `readAgentName`/`buildKgNodeLabel` 函数 + per-node fill/stroke/lineWidth + `KG_AGENT_PALETTE` 15 色暖色调 Agent 色环), `dagEditorialTokens.ts` (DAG 节点 6 类色系 + confidence tiers（medium `#a16207` WCAG AA）+ card 样式常量，预留未接入 GraphNodeCard) |
...
- 最近 full vitest 基线：`213 files / 2397 tests passed`
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-05-26 | 发布前 i18n / ErrorBoundary 收口 | `AppErrorBoundary` 改为 locale key + 内置中英文 fallback，覆盖 i18n 未初始化、raw key 和语言切换后刷新；`RoundtableAgentChat` 的 origin 摘要标签改走 `roundtable.agent_chat_origin_*`，并移除 `isZh` prop 链路。验证：frontend full `213 files / 2397 tests passed`，`npx tsc --noEmit`、`npm run lint`、`npm run build` 通过；i18n parity `2843/2843`；中文/英文主链路、AppErrorBoundary、语言切换和 Chromium/Firefox/WebKit smoke 通过 |
