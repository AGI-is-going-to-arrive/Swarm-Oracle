...
- **183 个测试文件** (`.test.ts` / `.test.tsx`) / **1965 tests**
...
| 日期 | 操作 | 说明 |
|------|------|------|
| 2026-05-11 | Sprint 5-6 Agent / Journal / Templates / Leaderboard 收口 | `/agents` favorites filter 改为 pressed-button group，Agent card 长 persona 按码点截断；`/agents/new` manual/document tab 补键盘与 PDF 错误态；persona import 使用后端返回的 string `identity_id` 并刷新库。`/me/journal` 已接 submit/resolve/calibration，resolve 后显示实际结果与 Brier 分；`/leaderboard` segment filters 同步 URL，retry/empty/loading 文案走 i18n 并遵守 reduced-motion。`InputView` 教育模板 picker、launch confirm、IME guard 和 `prefers-reduced-motion` 一并复核。真实验证：frontend full vitest `183 files / 1965 tests / 0 failed`，`tsc` 通过，`npm run build` 通过，i18n parity `2354=2354`；手动 Chromium browser spot-check 覆盖 `/`、`/me/journal`、`/agents`、`/agents/new`、identity inspector、`/leaderboard`、`/admin/setup` 与 375px mobile |
