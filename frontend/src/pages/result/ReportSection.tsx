...
export const ReportSection = React.memo(function ReportSection({ section, onOpenEvidence, index }: Props) {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-end mb-4">
        ...
        {section.evidence_refs && section.evidence_refs.length > 0 && (
          <button
            ...
            className="text-sm px-3 py-1.5 rounded-md bg-[color:var(--bg-hover)] hover:bg-[color:var(--bg-deep)] text-[color:var(--color-primary)] border border-[color:var(--border-default)] focus:outline-none focus:ring-2 focus:ring-[color:var(--color-ring)] transition-colors motion-reduce:transition-none"
            aria-label={isZh ? '查看引用的证据' : 'View cited evidence'}
          >
      ...
      <div className="prose prose-sm md:prose-base max-w-none text-[color:var(--text-secondary)]">
      ...
      {section.charts && section.charts.length > 0 && (
        <div className="mt-4 p-4 bg-[color:var(--bg-hover)] rounded border border-[color:var(--border-subtle)] text-sm text-[color:var(--text-muted)] italic">
          {isZh ? '图表可视化暂不可用。' : 'Chart visualization is not available yet.'}
        </div>
      )}
    </section>
  );
