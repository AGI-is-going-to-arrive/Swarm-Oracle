 ...
 *
 * Uses html2canvas for screenshots and canvas-based recording for GIFs.
...
/**
 * Capture screenshots from DOM elements or canvases.
 */
export async function captureElementBlob(
  ...
  await settleFrames();
  await settleElementImages(el as Element);
  ...
  const tryHtml2Canvas = async () => {
    ...
    const colorOverrides = collectColorStyleOverrides(el as Element);
  ...
  try {
    ...
    if (captureElementFirst) {
      const fallbackBlob = await captureElementBlobViaSvgFallback(el as Element);
...
async function normalizeDownloadBlob(blob: Blob, filename: string): Promise<Blob> {
...
export function useScreenCapture(options: UseScreenCaptureOptions = {}): UseScreenCaptureReturn {
  ...
  const captureScreenshot = useCallback(async (options?: CaptureRequestOptions) => {
    ...
    try {
      ...
      const blob = options?.captureBlob
        ? await options.captureBlob()
        : options?.selectors?.length
          ? await captureFirstAvailableBlob(options.selectors, captureTarget)
          : await captureElementBlob(targetSelector, captureTarget);
      if (!blob) {
        setStatus('error');
        return;
      }
      ...
      console.error('[useScreenCapture] Screenshot failed:', err);
      setStatus('error');
      setTimeout(() => setStatus('idle'), 2000);
    }
  }, [selector]);
  ...
  const captureGIF = useCallback(async (options?: CaptureRequestOptions) => {
    ...
    try {
      ...
      for (let i = 0; i < frameCount; i++) {
        await new Promise((r) => setTimeout(r, gifFrameInterval));
        const blob = await new Promise<Blob | null>((resolve) => {
      ...
      try {
        ...
        for (const frameBlob of frames) {
          ...
          gif.addFrame(ctx, { copy: true, delay: gifFrameInterval });
        }
      ...
      console.error('[useScreenCapture] GIF recording failed:', err);
      setStatus('error');
      setTimeout(() => setStatus('idle'), 2000);
    } finally {
      recordingRef.current = false;
