...
describe('performance budget config', () => {
  it('covers every explicitly named manual chunk, the shared vendor chunk, and React Flow auto-split chunks', () => {
    ...
    const expectedBudgets = [
      ...
      expectedChunkBudget('graphTraversal', { label: 'React Flow graph traversal/card chunk', maxBytes: 24 * 1024, maxGzipBytes: 8 * 1024 }),
      expectedChunkBudget('graphTraversal', { label: 'React Flow graph traversal/card chunk', maxBytes: 24 * 1024, maxGzipBytes: 8 * 1024, variant: 'legacy' }),
