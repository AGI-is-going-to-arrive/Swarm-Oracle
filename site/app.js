...
(function () {
  ...
  var FEATURES = [
    ...
    { id: "F42", img: "25-model-profiles.png", hero: false, zt: "模型配置", et: "Model Profiles",
      zd: "保存多套 LLM 接入信息（服务商、Base URL、模型、密钥、限速、并发、三态能力覆盖和原生搜索上游声明）；官方裸 /v1 可按 Responses 形态探测，测试连接仍分开显示普通连通性和模型 native 搜索。", ed: "Save several LLM setups (provider, base URL, model, key, rate limits, concurrency, tri-state capability overrides, and native-search upstream declarations); official bare /v1 endpoints are probed as Responses form while test connection still separates ordinary connectivity from model-native search." },
