...
(function () {
  ...
  var FEATURES = [
    ...
    { id: "F42", img: "25-model-profiles.png", hero: false, zt: "模型配置", et: "Model Profiles",
      zd: "保存多套 LLM 接入信息（服务商、Base URL、模型、密钥、限速、并发和三态能力覆盖，自动=跟随 provider 能力）；/admin/setup 测试成功的 profile 会让首页识别为已配置 LLM。", ed: "Save several LLM setups (provider, base URL, model, key, rate limits, concurrency and tri-state capability overrides, where auto follows provider capability); profiles tested from /admin/setup make the home page recognize LLM access as configured." },
