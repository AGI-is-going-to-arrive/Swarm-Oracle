...
(function () {
  ...
  var FEATURES = [
    ...
    { id: "F51", img: "21-simulation.png", hero: false, zt: "Agent 状态与分支感知记忆", et: "Agent State and Branch-Aware Memory",
      zd: "Agent 会收到与当前场景、所选分支和轮次截点相符的上下文；可核查的记忆引用使用哈希标识，兄弟分支和截点后的未来保持隔离。这里描述的是场景内的上下文与个体记忆差异，不承诺跨场景长期人格连续性。", ed: "Agents receive context scoped to the current scenario, selected branch, and round cutoff. Inspectable memory references use hashed identifiers, while sibling branches and post-cutoff future data stay isolated. This describes in-scenario context and per-agent memory differences, not guaranteed long-term personality continuity across scenarios." },
