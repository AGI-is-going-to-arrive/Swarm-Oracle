...
(function () {
  ...
  var FEATURES = [
    ...
    { id: "F51", img: "21-simulation.png", hero: false, zt: "Agent 决策、跨轮反馈与分支记忆", et: "Agent Decisions, Round Feedback, and Branch Memory",
      zd: "每个 Agent 先形成只含可审计事实依据的结构化决策，发言与动作共同消费同一决策；上一轮已验证或失败的推演内结果会进入下一轮状态转换与重规划。状态、上下文与哈希记忆引用仍按场景、分支和轮次截点隔离；这些字段不是完整领域世界模型，也不承诺跨场景长期人格连续性。", ed: "Each Agent first forms a structured decision containing auditable factual bases only, and both speech and action consume the same decision; verified or failed in-simulation outcomes feed the next round's state transition and replanning. State, context, and hashed memory references remain scoped by scenario, branch, and round cutoff; these fields are not a complete domain world model and do not guarantee long-term personality continuity across scenarios." },
    ...
    { id: "F21", img: "06-causal-map.png", hero: false, zt: "因果审查与行动账本", et: "Causal Review and Action Ledger",
      zd: "因果审查把事件、分叉和结局连成有向图，并可对照 Action Ledger 检查每轮动作、目标与回执；所选分支会拼接分叉前祖先轮次并保留可用来源坐标。这是对推演内部因果链的复盘，不是对现实世界因果关系的证明。", ed: "Causal Review connects events, forks, and endings in a directed graph and pairs with the Action Ledger to inspect each round's action, target, and receipt. The selected branch stitches pre-fork ancestor rounds and keeps available source coordinates. This reviews causality inside the simulation; it does not prove real-world causal relationships." },
    ...
    { id: "F53", img: "21-simulation.png", hero: false, zt: "原生社交行动世界", et: "Native Social Action World",
      zd: "Agent 可执行 POST、COMMENT、REACTION、FOLLOW、MUTE、SEARCH、TREND、REFRESH 与 IDLE；关注和静音会改变后续可见 Feed，动作、目标与结果写入 Action Ledger，供 Replay 与因果审查复核。", ed: "Agents can POST, COMMENT, REACTION, FOLLOW, MUTE, SEARCH, TREND, REFRESH, or IDLE. Following and muting change the Feed they see next, while actions, targets, and outcomes enter the Action Ledger for Replay and causal review." },
