...
(function () {
  ...
  var FEATURES = [
    ...
    { id: "F42", img: "25-model-profiles.png", hero: false, zt: "模型配置", et: "Model Profiles",
      zd: "保存多套 LLM 接入信息（服务商、Base URL、模型、密钥、限速、并发、三态能力覆盖和原生搜索上游声明），在推演或辩论前直接选择；官方裸 /v1 可按 Responses 形态探测，本地/自定义代理需显式声明上游，测试连接仍分开显示普通连通性和模型 native 搜索。", ed: "Save several LLM setups (provider, base URL, model, key, rate limits, concurrency, tri-state capability overrides, and native-search upstream declarations) and select one before a simulation or debate; official bare /v1 endpoints are probed as Responses form, local/custom proxies need an explicit upstream, and test connection still separates ordinary connectivity from model-native search." },
