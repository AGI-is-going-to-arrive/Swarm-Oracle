...
(function () {
  ...
  var FEATURES = [
    { id: "F01", img: "01-home.png", hero: false, zt: "首页提问框与开始推演", et: "Home Question Box and Start Simulation",
      zd: "在首页输入一个「如果……会怎样？」问题，确认问题和本局设置后进入多 Agent 推演；运行中会显示轮次、轮内发言进度和慢模型仍在工作的状态，真正失败或取消时未完成分支显示为已中断，问题、LLM 或预算未就绪时，开始按钮会显示原因。", ed: "Enter a “what if?” question on the home page, confirm the question and run settings, then start the multi-agent simulation; during the run it shows rounds, in-round speaking progress, and the still-working state for slow models. Truly failed or cancelled runs show unfinished branches as interrupted. When the question, LLM, or budget is not ready, the start button shows the reason." },
    ...
    { id: "F41", img: "23-full-report.png", hero: false, zt: "结果完整报告", et: "Result Full Report",
      zd: "结果页先露深读摘要；完整报告展开证据、AI 扮演角色访谈、后续观察指标和概率/阵营图表。重生成后原页更新，证据可跳回 replay。", ed: "The result page previews the deep read; the full report opens evidence, simulated-persona interviews, watch signals, and probability/faction charts. Regeneration updates in place, and evidence jumps back to replay." },
    ...
    { id: "F38", img: "01-home.png", hero: false, config: true, zt: "搜索增强推演", et: "Search-Augmented Simulation",
      zd: "打开后系统会在推演前做外部搜索并把相关片段注入角色提示，推荐路径优先用服务端已配置搜索，高级路径允许本轮用自己的 provider；这和模型 native 搜索分开判断。", ed: "When enabled, the system runs external search before simulating and injects relevant snippets into role prompts; the recommended path uses server-configured search, the advanced path lets you use your own provider for the round. This is separate from model-native search." },
    { id: "F48", img: "22-local-packs.png", hero: false, zt: "本地主题包", et: "Local Packs",
      zd: "首页的「本地主题包」收录一批双语预设场景，现在用类型分段和单一搜索框筛选，标签并入搜索；当前结果会自动预览，筛选状态显示结果数和清除入口。点一下模板即可填入问题和推荐设置。", ed: "The home page bundles bilingual preset packs, now filtered by genre segments plus one search box with tags folded into search; the current result previews automatically, and active filters show result count plus a clear action. One template click fills the question and suggested settings." },
    ...
    { id: "F42", img: "25-model-profiles.png", hero: false, zt: "模型配置", et: "Model Profiles",
      zd: "保存多套 LLM 接入信息（服务商、Base URL、模型、密钥、限速、并发、三态能力覆盖和原生搜索上游声明），在推演、辩论或结果页会客厅前直接选择；会客厅选择器默认收在高级设置里。官方裸 /v1 可按 Responses 形态探测，本地/自定义代理需显式声明上游，测试连接仍分开显示普通连通性和模型 native 搜索。", ed: "Save several LLM setups (provider, base URL, model, key, rate limits, concurrency, tri-state capability overrides, and native-search upstream declarations) and select one before a simulation, debate, or result-page chamber; chamber selection stays under Advanced settings by default. Official bare /v1 endpoints are probed as Responses form, local/custom proxies need an explicit upstream, and test connection still separates ordinary connectivity from model-native search." },
  ...
  var MODES = [
    { n: "01", img: ["21-simulation.png", "02-result.png"], zt: "多分支推演", et: "Multi-branch simulation",
      zd: "一个「如果……会怎样？」问题，由 AI 代理群体模拟多条故事线；推演页显示当前轮次、轮内 Agent 发言进度、整体进度、ETA 和首轮预热时长，慢模型只要仍有内容、进度活动或运行信号就提示仍在工作，真正失败、中断或取消时未完成分支显示为已中断，底部进度与回放时间线避开消息和控件，结果页用一句话回答原问题并给出置信度，每条终局叶子世界线带标题、归一化概率、故事摘要和直接回答。", ed: "One “what if?” question, simulated by a swarm of AI agents into multiple storylines; the simulation page shows current round, in-round Agent speaking progress, overall progress, ETA and first-round warmup elapsed time. Slow models stay in the still-working state while content, progress, or run signals continue; truly failed, interrupted, or cancelled runs show unfinished branches as interrupted. Bottom progress and replay timelines stay clear of messages and controls, and the result page answers the original question in one line with a confidence. Each terminal leaf worldline carries a title, normalized probability, story summary and a direct answer." },
