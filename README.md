...
当前验证口径（最近更新：2026-05-22 SimulationView completed / replay 收口）：

- backend：`python -m pytest -x --tb=short` 为 `3290 passed, 6 skipped`；`ruff check app/ tests/` 通过。
- frontend：本轮 `npm test` 为 `205 files / 2305 tests passed`；`npm run lint`、`npx tsc --noEmit -p tsconfig.app.json`、`npm run build` 通过；i18n key parity 为 `zh: 2786`、`en: 2786`、placeholder parity OK。
