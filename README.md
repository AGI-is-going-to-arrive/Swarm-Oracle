...
| Butterfly Effect | 已落地，支持即时 / 回溯 / 批量干预；回溯干预受 `counterfactual_replay` capability 控制，关闭或探针失败时前端不会提交回溯请求，后端同样返回 404；完成后会把可解释的干预效果写成只读 receipt |
...
| Replay & Import | 主模式与 Debate 均支持；主模式 Replay Trace 支持分页和分支过滤；scenario/story branch response 会回显 `replay_kind / replay_source_branch_id` 以保留 replay provenance；主模式 completed replay 会按 `visualization_enabled` 保持 Theater，单分支或单轮只显示静态标签；新生成的主模式和 Oracle replay payload 会剥掉 Agent 的 `agent_identity_id` 和 persona，只保留回放展示需要的字段；Debate replay 当前已补 `share -> readonly replay -> reload restore -> import` 完整 E2E 覆盖 |
...
当前验证口径（最近更新：2026-05-22 capability gate / 回溯干预收口）：

- backend：`python -m pytest -x -q --tb=short` 为 `3286 passed, 11 skipped`；`ruff check app/ tests/` 通过。
- frontend：本轮 `npx vitest run` 为 `206 files / 2318 tests passed`；`npx eslint src/ --max-warnings=0`、`npx tsc --noEmit -p tsconfig.app.json` 通过；i18n key parity 为 `zh: 2791`、`en: 2791`。
...
- 跨浏览器限制：`npm run e2e:cross-browser -- --url http://localhost:18928 --headless` 当前在 Firefox DirectorState 等待上超时，截图显示页面已进 completed simulation/theater 状态；因此这轮不能把 Firefox/WebKit 全矩阵写成已签收。
